...
class Profile extends CI_Controller {
	...
	{	
		...
		$whereCon['where']			 		= 	'usr.encrypt_id ="'.$this->session->userdata('SMS_HOSTEL_ADMIN_ID').'"';		
	...
	public function editprofile($editid='')
	{
		...
		$data['error'] 						= 	'';
                         ...
                         $subQuery           = "SELECT usr.*,ad.encrypt_id as address_id,ad.user_c_address ,ad.user_c_locality ,ad.user_c_city ,ad.user_c_state  ,ad.user_c_zipcode  FROM sms_users AS usr   LEFT JOIN `sms_user_address` AS ad ON ad.user_id = usr.encrypt_id where   usr.encrypt_id='".$editid."'  and usr.franchise_id = '" . $this->session->userdata('SMS_HOSTEL_ADMIN_FRANCHISE_ID') . "'
												 AND usr.school_id = '" . $this->session->userdata('SMS_HOSTEL_ADMIN_SCHOOL_ID') . "' 
												 AND usr.branch_id = '" . $this->session->userdata('SMS_HOSTEL_ADMIN_BRANCH_ID') . "' " ;
		...
		if($this->input->post('SaveChanges')):
			...
			if($this->form_validation->run() && $error == 'NO'):
				...
				$Adparam['updated_by']				=	$this->session->userdata('SMS_HOSTEL_ADMIN_ID');
                                   $this->common_model->edit_data('user_address',$Adparam,$data['profileuserdata']['address_id']);
                               else:
                                  $Adparam['creation_date'] = currentDateTime();
                $Adparam['created_by']    = $this->session->userdata('SMS_HOSTEL_ADMIN_ID');
                $Adparam['status']        = 'Y';
				...
				if($this->input->post('new_password')):
					...
					if(get_cookie('SMSHOSTELADMINRememberMe') == 'YES'):
						setcookie("SMSHOSTELADMINUserEmail",$data['profileuserdata']['user_email'],time()+60*60*24*100,'/');
						setcookie("SMSHOSTELADMINUserPass",$NewPassword,time()+60*60*24*100,'/');
				...
				if($result):
					...
					$this->session->set_userdata(array(
											...
											'SMS_HOSTEL_ADMIN_SLUG'=>$result['branch_slug'],
				...
				redirect($this->session->userdata('SMS_HOSTEL_ADMIN_PATH').$this->router->fetch_class().'/index');
			endif;
		endif;
