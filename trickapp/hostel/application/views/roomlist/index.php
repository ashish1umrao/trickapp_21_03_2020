<ol class="breadcrumb">
  ...
  <?php if($add_data =='Y'): ?>
  <li class="pull-right"><a href="{FULL_SITE_URL}{CURRENT_CLASS}/addeditdata" class="btn btn-default">Add room </a></li>
  <?php endif; ?>
</ol>
{message}
      ...
      <table class="table table-striped b-t b-light">
        ...
        <tbody>
          ...
          <tr class="<?php if($i%2 == 0): echo 'odd'; else: echo 'even'; endif; ?>">
            <td><?=$i++?></td>
            <td><?=stripslashes($ALLDATAINFO['room_name'])?></td>
            <td><?=stripslashes($ALLDATAINFO['room_type_name'])?></td>
              <td><?=stripslashes($ALLDATAINFO['available_bed'])?></td>
            
       <td><?=showStatus($ALLDATAINFO['status'])?></td>
        <?php if($edit_data == 'Y'and $ALLDATAINFO['available_bed']): ?>
         <td align="right"><div class="btn-group"> <a class="btn btn-primary " href="{FULL_SITE_URL}{CURRENT_CLASS}/allocate/<?=$ALLDATAINFO['encrypt_id']?>">Allocate </a></td>
            <?php else:  ?>
            ...
            <td><div class="btn-group"> <a class="btn btn-success dropdown-toggle" data-toggle="dropdown" href="javascript:void(0);">Action <i class="fa fa-arrow-down icon-white"></i></a>
                  <ul class="dropdown-menu">
                    ...
                    <?php endif; endif; ?>
                     <li class="divider"></li>
                     <?php if($view_data == 'Y'): ?>
                     <li><a href="javascript:void(0);" class="view-details-data" title="Room Details" data-id="<?=$ALLDATAINFO['encrypt_id']?>"><i class="fa fa-gear"></i> View details</a></li>
                      <?php endif; ?>
                  </ul>
                </div>
            </td>
