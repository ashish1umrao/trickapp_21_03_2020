...
if(isset($firstSlug)):
	...
	if($fSlugData):
		...
		if($secondSlug == 'dashboard'):
			...
			if(isset($thirdSlug)):
				$tSlugQuery 				= 	$db->select('encrypt_id')->from('sms_hostel')->where("hostel_slug = '".$thirdSlug."' AND status = 'Y'")->get();
				$tSlugData					= 	$tSlugQuery->result(); 
                            
				if($tSlugData):  
					if($fourthSlug == 'dashboard'):
                                                
						$route[$firstSlug.'/'.$secondSlug.'/'.$thirdSlug.'/dashboard'] 		= 	'admin/dashboard/$1';
					else:	
                                            
                                        
						$newCurUrl			=	explode('/'.$firstSlug.'/'.$secondSlug.'/'.$thirdSlug.'/',$_SERVER['REQUEST_URI']);
						...
						$route[$firstSlug.'/'.$secondSlug.'/'.$thirdSlug.'/'.$classFunction[0]] 	= 	$classFunction[0].'/$1'; 
					endif; 
					...
					if($classFunction[0]):
						$defaultClassArray	=	array('profile','franchising','school','branch','designation','subadmin');
						$classArray			=	explode('/',$classFunction[0]);
						if(isset($classArray[0])):
							$classQuery 	= 	$db->select('encrypt_id')->from('sms_module')->where("module_name = '".$classArray[0]."' AND show_type = 'Hostel'")->get();
							$classData		= 	$classQuery->result();
							$subClassQuery 	= 	$db->select('encrypt_id')->from('sms_child_module')->where("module_name = '".$classArray[0]."' AND show_type = 'Hostel'")->get();
