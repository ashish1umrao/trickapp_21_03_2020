<ol class="breadcrumb">
  ...
  <?php if($add_data =='Y'): ?>
  <li class="pull-right"><a href="{FULL_SITE_URL}{CURRENT_CLASS}/addeditdata" class="btn btn-default">Send Sms</a></li>
  <?php endif; ?>
</ol>
{message}
      ...
      <table class="table table-striped b-t b-light">
        ...
        <tbody>
          <?php if($ALLDATA <> ""): $i=$first; foreach($ALLDATA as $ALLDATAINFO): 
            if(!empty($ALLDATAINFO['Number'])){
            if(!empty($ALLDATAINFO['MessageId'])){
              $Status = "Deliver";
            }else{
              $Status = "Sent";
            }
          ...
          <tr class="<?php if($i%2 == 0): echo 'odd'; else: echo 'even'; endif; ?>">
            ...
            <td><textarea rows="5" cols="40"><?=stripslashes($ALLDATAINFO['content'])?></textarea></td>
            <td><?=stripslashes($ALLDATAINFO['login_type'])?></td>
            <td><?=$Status; ?></td>  
