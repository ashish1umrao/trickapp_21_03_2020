<ol class="breadcrumb">
  ...
  <li class="active">Manage Home Work</li>
  <?php if($add_data =='Y'): ?>
  <?php /*?><li class="pull-right"><a href="{FULL_SITE_URL}{CURRENT_CLASS}/addeditdata" class="btn btn-default">Add Home Work</a></li><?php */?>
  <?php endif; ?>
</ol>
{message}
      ...
      <table class="table table-striped b-t b-light">
        ...
        <tbody>
          ...
          <tr class="<?php if($i%2 == 0): echo 'odd'; else: echo 'even'; endif; ?>">
            ...
            <td><?=stripslashes($ALLDATAINFO['home_work_title'])?></td>
            <td><textarea rows="3" cols="30"><?=stripslashes($ALLDATAINFO['home_work_content'])?>
</textarea></td>
            <td><?php if(!empty($ALLDATAINFO['home_work_file'])){ ?>
              <img src="<?=$ALLDATAINFO['home_work_file']; ?>" width="100" height="100" alt="Home work image"> 
            <?php }else { echo "---"; } ?>
            </td>
          
             <td><?=date('d/m/Y',strtotime($ALLDATAINFO['home_work_from_date']))?></td>
          
             <td><?=date('d/m/Y',strtotime($ALLDATAINFO['home_work_to_date']))?></td>
