<ol class="breadcrumb">
  ...
  <li class="active">Manage Student leave Request</li>
  <?php if($add_data =='Y'): ?>
  <?php /*?><li class="pull-right"><a href="{FULL_SITE_URL}{CURRENT_CLASS}/addeditdata" class="btn btn-default">Manage Student leave Request</a></li><?php */?>
  <?php endif; ?>
</ol>
{message}
  ...
  <div class="panel panel-default">
    ...
    <div class="table-responsive">
      <table class="table table-striped b-t b-light">
        ...
        <tbody>
          <?php if($ALLDATA <> ""): $i=$first; foreach($ALLDATA as $ALLDATAINFO):  //echo "<pre>"; print_r($ALLDATAINFO); ie;?>
          <tr class="<?php if($i%2 == 0): echo 'odd'; else: echo 'even'; endif; ?>">
            ...
            <td><?=stripslashes($ALLDATAINFO['user_type'])?></td>
            <td><?=stripslashes($ALLDATAINFO['leave_type'])?></td>
            <td><?=date('d/m/Y',strtotime($ALLDATAINFO['from_date']))?></td>
            <td><?=date('d/m/Y',strtotime($ALLDATAINFO['to_date']))?></td>
            <td><textarea rows="3"><?=stripslashes($ALLDATAINFO['leave_reason'])?></textarea></td>
            <td><?=stripslashes($ALLDATAINFO['leave_status'])?></td>
