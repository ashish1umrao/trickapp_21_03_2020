<ol class="breadcrumb">
  ...
  <li class="active">Manage event details</li>
  <?php if($add_data =='Y'): ?>
  <li class="pull-right"><a href="{FULL_SITE_URL}{CURRENT_CLASS}/addeditdata" class="btn btn-default">Add event details</a></li>
  <?php endif; ?>
</ol>
{message}
        ...
        <tbody>
          ...
          <tr class="<?php if($i%2 == 0): echo 'odd'; else: echo 'even'; endif; ?>">
            ...
            <td><?=stripslashes($ALLDATAINFO['purpose'])?></td>
                 <td><?=stripslashes($ALLDATAINFO['venue'])?></td>
            <td><?=YYMMDDtoDDMMYY($ALLDATAINFO['from_date'])?></td>
             <td><?=YYMMDDtoDDMMYY($ALLDATAINFO['to_date'])?></td>
               <td><?=stripslashes($ALLDATAINFO['time'])?></td>
