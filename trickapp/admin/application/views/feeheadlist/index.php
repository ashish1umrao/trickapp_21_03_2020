...
<ol class="breadcrumb">
  ...
  <?php if($add_data =='Y'): ?>
  <li class="pull-right"><a href="{FULL_SITE_URL}{CURRENT_CLASS}/addeditdata" class="btn btn-default">Add fee head</a></li>
  <?php endif; ?>
</ol>
{message}
      ...
      <table class="table table-striped b-t b-light">
        <thead>
          <tr>
            ...
            <th>Head name</th>
            <th>Frequency</th>
            <?php if($STARTMONTH <> ""): $month	=	date("m", strtotime($STARTMONTH)); 
				  for ($i = ($month-1); $i < (12+($month-1)); ++$i):
			?>
            <th><?php echo date("M", strtotime("January +$i months")); ?></th>
            <?php endfor; endif; ?>
        ...
        <tbody>
          ...
          <tr class="<?php if($i%2 == 0): echo 'odd'; else: echo 'even'; endif; ?>">
            ...
            <td><?=stripslashes($ALLDATAINFO['fee_frequency_name'])?></td>
               <td><?=stripslashes($ALLDATAINFO['month_1st_fee'])?></td>
                 <td><?=stripslashes($ALLDATAINFO['month_2nd_fee'])?></td>
                   <td><?=stripslashes($ALLDATAINFO['month_3rd_fee'])?></td>
                     <td><?=stripslashes($ALLDATAINFO['month_4th_fee'])?></td>
                       <td><?=stripslashes($ALLDATAINFO['month_5th_fee'])?></td>
                       
                         <td><?=stripslashes($ALLDATAINFO['month_6th_fee'])?></td>
                           <td><?=stripslashes($ALLDATAINFO['month_7th_fee'])?></td>
                             <td><?=stripslashes($ALLDATAINFO['month_8th_fee'])?></td>
                               <td><?=stripslashes($ALLDATAINFO['month_9th_fee'])?></td> 
                                 <td><?=stripslashes($ALLDATAINFO['month_10th_fee'])?></td>
                                   <td><?=stripslashes($ALLDATAINFO['month_11th_fee'])?></td>
                                                       <td><?=stripslashes($ALLDATAINFO['month_12th_fee'])?></td>
