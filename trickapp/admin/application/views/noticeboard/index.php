<ol class="breadcrumb">
  ...
  <li class="active">Manage noticeboard details</li>
  
  <?php if($add_data =='Y'): ?>
  <li class="pull-right"><a href="{FULL_SITE_URL}{CURRENT_CLASS}/addeditdata" class="btn btn-default">Add noticeboard details</a></li>
  <?php endif; ?>
      ...
      <table class="table table-striped b-t b-light">
        ...
        <tbody>
          ...
          <tr class="<?php if($i%2 == 0): echo 'odd'; else: echo 'even'; endif; ?>">
            <td><?=$i++?></td>
             <td><?php if($ALLDATAINFO['image']):  ?><img src="<?=$ALLDATAINFO['image']?>" alt="Smiley face" height="42" width="42"> <?php endif; ?> </td>
            <td> <?=stripslashes($ALLDATAINFO['visibility'])?></td>
          
            <td><?=$ALLDATAINFO['message']?></td>
            <td><?=stripslashes($ALLDATAINFO['readed'])?></td>
            <td><?=stripslashes($ALLDATAINFO['class_name'])?></td>
            <td><?=stripslashes($ALLDATAINFO['class_section_name'])?></td>
           <?php /*?> <td><?=showStatus($ALLDATAINFO['status'])?></td>
            <?php if($edit_data == 'Y' || $delete_data == 'Y' || $view_data == 'Y'): ?>
