...
<ol class="breadcrumb">
  ...
  <li class="active">Manage non teaching staff attendance details</li>
</ol>
<span class="right " style="float:right;">
   <?php if($edit_data == 'Y' && $ALLDATA <> ""):  ?>
...
<div class="table-agile-info">
  <div class="panel panel-default">
      ...
      <div class="col-sm-8 m-b-xs">
       ...
       <a class="btn btn-success "  href="{ASSET_URL}demo_doc/staff_attendance1.csv" download>Demo Upload CSV<i class="fa fa-upload" aria-hidden="true"></i></a>
      </div>
      <div class="col-sm-3"  >
        <input type="file" name="staffFile" id="staffFile"    accept=".csv" class="form-control required" style="height:auto;">
      </div> 
      <div class="col-sm-1">
         <input type="submit" name="SaveTeacherExcelUpload" id="SaveTeacherExcelUpload" value="Submit" class="btn btn-sm btn-default UploadData" />
      </div>
    </div>
        </form>
      <?php endif;  ?>
    ...
    <div class="row w3-res-tb">
      <div class="col-sm-6 m-b-xs">
         <select name="staff_id" id="staff_id" class="input-sm form-control  required w-sm inline v-middle">
            <option value="">Select staff</option>
            <option value="All" <?php if($staffid == 'All'){ echo  'selected="selected"'; } ?>>All</option>
            <?php  if($TEACHERDATA <> ""): foreach ($TEACHERDATA as $staffInfo): ?>
            <option value="<?php echo stripcslashes($staffInfo['encrypt_id']); ?>" <?php if($staffid == $staffInfo['encrypt_id']){ echo  'selected="selected"'; } ?> > <?php echo stripcslashes($staffInfo['user_f_name'].' '.$staffInfo['user_m_name'].' '.$staffInfo['user_l_name']); ?></option>
    ...
    <div class="table-responsive">
      <table class="table table-striped b-t b-light">
        ...
        <tbody>
          <?php if($ALLDATA <> ""): $i=1; foreach($ALLDATA as $ALLDATAINFO): //echo "<pre>"; print_r($ALLDATAINFO['attendance']); die; ?>
          <tr class="<?php if($i%2 == 0): echo 'odd'; else: echo 'even'; endif; ?>">
            <td><?=$i++?></td>
            <td><?=stripslashes($ALLDATAINFO['user_f_name'])?>  <?=stripslashes($ALLDATAINFO['user_m_name'])?>  <?=stripslashes($ALLDATAINFO['user_l_name'])?></td>
           <td> <?=$ALLDATAINFO['user_mobile'] ?></td>
               <td> <?=$ALLDATAINFO['employee_id'] ?></td>
            ...
            <td><?=stripslashes($ALLDATAINFO['attendance'])?></td>
          </tr>
          <?php endforeach; else: ?>
            <tr>
...
$('#end_date_attdn').on('change', function (ev) {
   var end_date = $(this).val();
    ...
    var teacherid = "<?php echo $staffid ?>"; 
    if(teacherid == 'All') {
        $("#hide_date").hide();
           $("#show_date").show(); 
       }else{
