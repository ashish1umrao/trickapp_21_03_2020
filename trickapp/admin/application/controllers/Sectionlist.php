...
class Sectionlist extends CI_Controller {
    ...
    public function addeditdata($editid = '') {
        ...
        if ($this->input->post('SaveChanges')):
			...
			if($data['EDITDATA']<>""): 
			$editID = $data['EDITDATA']['class_section_id'];
			//echo $editID; die;
			          $CLASSID         =  $this->input->post('class_id');
					  $CLASSSECTIONNAME  =  $this->input->post('class_section_name');
					  $CLASSTRACHERID  =  $this->input->post('class_teacher_id');
					  $ASSIGNTEACHERID	   = 	$this->common_model->getsectionwiseassignteacher($CLASSID, $CLASSSECTIONNAME,$CLASSTRACHERID); 
				
					if($ASSIGNTEACHERID == ""){
					$param['class_teacher_id']         = addslashes($this->input->post('class_teacher_id'));
	
						}else{
					$this->session->set_flashdata('alert_error',lang('TEACHERerror'));
					redirect(correctLink('sectionListAdminData',$this->session->userdata('SMS_ADMIN_PATH').$this->router->fetch_class().'/addeditdata/'.$editID.''));
			} 
			else:
					$CLASSID         =  $this->input->post('class_id');
					  $CLASSSECTIONNAME  =  $this->input->post('class_section_name');
					...
					$this->session->set_flashdata('alert_error',lang('TEACHERerror'));
			       redirect(correctLink('sectionListAdminData', $this->session->userdata('SMS_ADMIN_PATH') . $this->router->fetch_class() . '/addeditdata'));
