...
class Holidaylist extends CI_Controller {
	 ...
	 * * Purpose  : This function used for holidaylist
	...
	{	
		...
		if($this->input->get('searchValue')):
			...
			$whereCon['like']		 		= 	"(h.purpose LIKE '%".$sValue."%' )";
		...
		$this->session->set_userdata('holidaylistAdminData',currentFullUrl());
		$qStringdata						=	explode('?',currentFullUrl());
		$config['suffix']					= 	$qStringdata[1]?'?'.$qStringdata[1]:'';
		$tblName 							= 	'holiday as h';
		$con 								= 	'';
		$config['total_rows'] 				= 	$this->admin_model->SelectHolidayData('count',$tblName,$whereCon,$shortField,'0','0');
       ...
       if ($this->uri->segment(getUrlSegment())):
		...
		$data['ALLDATA'] 					= 	$this->admin_model->SelectHolidayData('data',$tblName,$whereCon,$shortField,$config['per_page'],$page); 

		$this->layouts->set_title('Holiday details');
		$this->layouts->admin_view('holidaylist/index',array(),$data);
	}	// END OF FUNCTION
	...
	{		
		...
		if($editid):
			...
			$data['EDITDATA']		=	$this->common_model->get_data_by_encryptId('holiday',$editid);
		else:
		...
		if($this->input->post('SaveChanges')):
			...
			$this->form_validation->set_rules('startdate', 'lang:Start date', 'trim|required');
			$this->form_validation->set_rules('enddate', 'lang:End date', 'trim');
	...
	$params['startdate']		= 	DDMMYYtoYYMMDD($this->input->post('startdate'));
        $params['enddate']		= 	DDMMYYtoYYMMDD($this->input->post('enddate'));
				...
				if($this->input->post('CurrentDataID') ==''):
					...
					$alastInsertId				=	$this->common_model->add_data('holiday',$params);
					
					$Uparam['encrypt_id']		=	manojEncript($alastInsertId);
					$Uwhere['holiday_id']=	$alastInsertId;
					$this->common_model->edit_data_by_multiple_cond('holiday',$Uparam,$Uwhere);
					...
					$this->common_model->edit_data('holiday',$params,$TDPROGId);
					$this->session->set_flashdata('alert_success',lang('updatesuccess'));
				endif;
				
				redirect(correctLink('holidaylistAdminData',$this->session->userdata('SMS_ADMIN_PATH').$this->router->fetch_class().'/index'));
			endif;
		endif;
		
		$this->layouts->set_title('Edit Holiday Details');
		$this->layouts->admin_view('holidaylist/addeditdata',array(),$data);
	}	// END OF FUNCTION	
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->edit_data('holiday',$param,$changeStatusId);
