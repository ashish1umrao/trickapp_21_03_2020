...
class Createexam extends CI_Controller {
	...
	{			
		...
		$whereCon['where']	= "se.franchise_id = '" . $this->session->userdata('SMS_ADMIN_FRANCHISE_ID') . "' 
							AND se.school_id = '" . $this->session->userdata('SMS_ADMIN_SCHOOL_ID') . "' 
							AND se.branch_id = '" . $this->session->userdata('SMS_ADMIN_BRANCH_ID') . "' 
							AND se.board_id = '" . $this->session->userdata('SMS_ADMIN_BOARD_ID') . "'";		
		$shortField 		= 	'se.id ASC';
		...
		$tblName 							= 	'sms_exam as se';
		$con 								= 	'';
		$config['total_rows'] 				= 	$this->admin_model->SelectExamName('count',$tblName,$whereCon,$shortField,'0','0');
       ...
       if ($this->uri->segment(getUrlSegment())):
		...
		$data['ALLDATA'] 					= 	$this->admin_model->SelectExamName('data',$tblName,$whereCon,$shortField,$config['per_page'],$page); 
       // print "<pre>"; print_r($data['ALLDATA']); die;     
		$this->layouts->set_title('Set exam term details');
		$this->layouts->admin_view('createexam/index',array(),$data);
	}	// END OF FUNCTION
	...
	{	
		...
		if($editid):
			...
			$data['EDITDATA']		=	$this->common_model->get_data_by_encryptId('sms_exam',$editid);
		...
		if($this->input->post('SaveChanges')):
			...
			if($this->form_validation->run() && $error == 'NO'):   
				$param['term_id']		= 	addslashes($this->input->post('term_id'));				
				...
				if($this->input->post('CurrentDataID') ==''):
					...
					$param['status']			=	'Y';
				   	$alastInsertId				=	$this->common_model->add_data('sms_exam',$param);					
										
					$Uparam['encrypt_id']		=	manojEncript($alastInsertId);
					$Uwhere['id']	=	$alastInsertId;
					$this->common_model->edit_data_by_multiple_cond('sms_exam',$Uparam,$Uwhere);
					$this->session->set_flashdata('alert_success',lang('addsuccess'));
				else:
					$subjectHeadId				=	$this->input->post('CurrentDataID');
					$param['update_date']		=	currentDateTime();
					$param['updated_by']		=	$this->session->userdata('SMS_ADMIN_ID');
					$this->common_model->edit_data('sms_exam',$param,$subjectHeadId);
		...
		$this->layouts->set_title('Edit Exam details');
		$this->layouts->admin_view('createexam/addeditdata',array(),$data);
	}	// END OF FUNCTION	
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->edit_data('sms_exam',$param,$changeStatusId);
