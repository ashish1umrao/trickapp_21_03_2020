...
class Subjecthead extends CI_Controller {
	...
	{	
		...
		$whereCon['where']		 			= 	"subh.franchise_id = '".$this->session->userdata('SMS_ADMIN_FRANCHISE_ID')."' 
												 AND subh.school_id = '".$this->session->userdata('SMS_ADMIN_SCHOOL_ID')."' 
												 AND subh.branch_id = '".$this->session->userdata('SMS_ADMIN_BRANCH_ID')."' 
												 AND subh.board_id = '".$this->session->userdata('SMS_ADMIN_BOARD_ID')."'";		
		$shortField 						= 	'subh.subject_head_id ASC';
		...
		$tblName 							= 	'subject_head as subh';
		$con 								= 	'';
		$config['total_rows'] 				= 	$this->admin_model->SelectSubjectHeadData('count',$tblName,$whereCon,$shortField,'0','0');
       ...
       if ($this->uri->segment(getUrlSegment())):
		...
		$data['ALLDATA'] 					= 	$this->admin_model->SelectSubjectHeadData('data',$tblName,$whereCon,$shortField,$config['per_page'],$page); 
             
		$this->layouts->set_title('Manage subject head details');
		$this->layouts->admin_view('subjecthead/index',array(),$data);
	}	// END OF FUNCTION
	...
	{		
		...
		if($editid):
			...
			$data['EDITDATA']		=	$this->common_model->get_data_by_encryptId('subject_head',$editid);
		else:
		...
		if($this->input->post('SaveChanges')):
			$error					=	'NO';
			$this->form_validation->set_rules('subject_head_name', 'Subject head name', 'trim|required');
			$this->form_validation->set_rules('subject_head_short_name', 'Subject head short name', 'trim|required');
			
			if($this->form_validation->run() && $error == 'NO'):   
			
				$param['subject_head_name']		= 	addslashes($this->input->post('subject_head_name'));
				$param['subject_head_short_name']= 	addslashes($this->input->post('subject_head_short_name'));
				...
				if($this->input->post('CurrentDataID') ==''):
					...
					$alastInsertId				=	$this->common_model->add_data('subject_head',$param);
					
					$Uparam['encrypt_id']		=	manojEncript($alastInsertId);
					$Uwhere['subject_head_id']	=	$alastInsertId;
					$this->common_model->edit_data_by_multiple_cond('subject_head',$Uparam,$Uwhere);
					$this->session->set_flashdata('alert_success',lang('addsuccess'));
				else:
					$subjectHeadId				=	$this->input->post('CurrentDataID');
					$param['update_date']		=	currentDateTime();
					$param['updated_by']		=	$this->session->userdata('SMS_ADMIN_ID');
					$this->common_model->edit_data('subject_head',$param,$subjectHeadId);
		...
		$this->layouts->admin_view('subjecthead/addeditdata',array(),$data);
	}	// END OF FUNCTION	
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->edit_data('subject_head',$param,$changeStatusId);
