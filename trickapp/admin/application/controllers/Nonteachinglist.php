...
class Nonteachinglist extends CI_Controller {
	...
	{	
		...
		$whereCon['where']		 			= 	"user.franchise_id = '".$this->session->userdata('SMS_ADMIN_FRANCHISE_ID')."'
												 ...
												 AND user.branch_id = '".$this->session->userdata('SMS_ADMIN_BRANCH_ID')."' AND user.user_type != 'Teacher'  AND user.user_type != 'Parent'";		
		$shortField 						= 	'user.user_f_name ASC';
		
		$this->load->library('pagination');
		$config['base_url'] 				= 	$this->session->userdata('SMS_ADMIN_PATH').$this->router->fetch_class().'/index';
		$this->session->set_userdata('nonteachingListAdminData',currentFullUrl());
		$qStringdata						=	explode('?',currentFullUrl());
		$config['suffix']					= 	$qStringdata[1]?'?'.$qStringdata[1]:'';
		$tblName 							= 	'users as user';
		$con 								= 	'';
		$config['total_rows'] 				= 	$this->admin_model->SelectNonTeachingStaffListData('count',$tblName,$whereCon,$shortField,'0','0');
       ...
       if ($this->uri->segment(getUrlSegment())):
		...
		$data['ALLDATA'] 					= 	$this->admin_model->SelectNonTeachingStaffListData('data',$tblName,$whereCon,$shortField,$config['per_page'],$page); 

		$this->layouts->set_title('Manage staff  details');
		$this->layouts->admin_view('nonteachinglist/index',array(),$data);
	}	// END OF FUNCTION
	...
	{	$data['userId'] =  	$editid ;			
		...
		$subQuery					=	"SELECT encrypt_id, subject_name FROM sms_subject
		...
		$data['STATEDATA']			=	$this->common_model->get_data_by_query('multiple',$StateQuery); 
                ...
                $userTypeQuery					=	"SELECT user_type_name  FROM sms_user_types WHERE status = 'Y' ORDER BY user_type_name ASC";
               
		$data['USERTYPEDATA']			=	$this->common_model->get_data_by_query('multiple',$userTypeQuery); 
		...
		if($editid):
			...
			$uDeQuery				=	"SELECT user_gender,user_gender_short,user_marital_status,user_marital_short,user_blood_group,user_religion,user_nationality,
			                             user_dob,user_joining_date,user_pickup,user_drop,user_pic,user_dl_no,user_dl_image
		...
		if($this->input->post('SaveChanges')):
			...
			$this->form_validation->set_rules('user_type', 'Staff Name', 'trim|required');
			...
			//////////////	Non teaching staff's Correspondence Address
			...
			if($this->input->post('user_type') == 'Driver'):
				$this->form_validation->set_rules('user_dl_no', 'Driving Licence No', 'trim|required');
				$this->form_validation->set_rules('uploadimage1', 'Driving Licence Image', 'trim');
			endif;
			...
			if($this->form_validation->run() && $error == 'NO'):  //echo '<pre>'; print_r($_POST); die;
				...
				$Tparam['user_type']			= 	addslashes($this->input->post('user_type'));
				...
				if($this->input->post('user_type') == 'Driver'):
					$TDparam['user_dl_no']		= 	addslashes($this->input->post('user_dl_no'));
					$TDparam['user_dl_image']	= 	addslashes($this->input->post('uploadimage1'));
				endif;
				
				//////////////	Non teaching staff Address table
				...
				if($this->input->post('CurrentDataID') ==''):
					...
					//////////////	Non teaching staff Teacher Kyc table
		...
		$this->layouts->set_title('Edit staff details');
		$this->layouts->admin_view('nonteachinglist/addeditdata',array(),$data);
	}	// END OF FUNCTION	
	...
	************************************************************************/
	...
	{  
		...
		redirect(correctLink('nonteachingListAdminData',$this->session->userdata('SMS_ADMIN_PATH').$this->router->fetch_class().'/index'));
	}
			...
			if($viewData <> ""):
				...
				$html			.=	'<table class="table border-none">
								  <tbody>
									...
									<tr>
									  <td align="left" width="20%"><strong>Staff Type</strong></td>
									  <td align="left" width="30%">'.stripslashes($viewData['user_type']).'</td>
									...
									<tr>
										<td colspan="4">
										  <table class="table border-none">
											<thead>
											<tr>
											  <th align="left" colspan="4"><strong>Staff\'s correspondence address</strong></th>
											</tr>
											</thead>
											<tbody>
												<tr>
	...
	************************************************************************/
	...
	{ 
		...
		if($file_name): 
			...
			$return_file_name	=	$this->upload_crop_img->_upload_image($file_name,$tmp_name,'nonTeachingStaffImage',$newfilename,$imagefolder);
	...
	************************************************************************/
	public function print_in_pdf($printpdfId=''){	
		...
		if($printpdfId == ''): redirect(correctLink('nonteachingListAdminData',$this->session->userdata('SMS_ADMIN_PATH').$this->router->fetch_class().'/index')); endif;
		...
		$this->load->view('nonteachinglist/printinpdf',$data);
	...
	************************************************************************/
    ...
    public function addeditdoc($editId = '') {
        ...
        $docQuery         = "SELECT * FROM `sms_nonteaching_staff_doc_type` ";
        $data['DOCDATA']  = $this->common_model->get_data_by_query('multiple', $docQuery);
        $docSelectedQuery         = "SELECT * FROM `sms_nonteaching_staff_doc_type` where encrypt_id = '".$this->input->post('doc_type_id')."' ";
        ...
        $addQuery         = "SELECT *
										FROM sms_nonteaching_staff_document 
									 	WHERE 
										 user_id = '" .$editId. "'";
        $data['EDITDATA'] = $this->common_model->get_data_by_query('multiple', $addQuery);
         $alldataQuery         = "SELECT doc.doc_type , tec.* FROM sms_nonteaching_staff_document AS tec  
LEFT  JOIN  `sms_nonteaching_staff_doc_type` AS doc  ON doc.encrypt_id =  tec.doc_type_id  WHERE user_id = '" . $editId . "'";
        ...
        if ($this->input->post('SaveChanges')):
            ...
            if ($file_name):
                ...
                $this->load->library("upload_crop_img");
                    ...
                    $return_file_name = $this->upload_crop_img->_upload_image($file_name, $tmp_name, 'nonteachingstaffDocument', $newfilename, $imagefolder);
 ...
 endforeach;
                ...
                if (($this->input->post('CurrentDataID') == '') and  ($edit == '')):
                    ...
                    $Param['status']        = 'Y';
                    $teclastInsertId     = $this->common_model->add_data('nonteaching_staff_document', $Param);

                    $Sparam['encrypt_id']        =manojEncript($teclastInsertId);
                    $Swhere['staff_document_id'] = $teclastInsertId;
                    $this->common_model->edit_data_by_multiple_cond('nonteaching_staff_document', $Sparam, $Swhere);
                    ...
                    $this->common_model->edit_data_by_multiple_cond('nonteaching_staff_document', $Param, $Where);
        ...
        $this->layouts->set_title('Add/edit staff documents details');
        $this->layouts->admin_view('nonteachinglist/addeditdoc', array(), $data);
    }
