...
class Gradescale extends CI_Controller {
	...
	{			
       ...
       // print "<pre>"; print_r($data['ALLDATA']); die;     
		...
		$this->layouts->admin_view('gradescale/index',array(),$data);
	}	// END OF FUNCTION
	...
	{	
		...
		if($editid):
			...
			$data['EDITDATA']		=	$this->common_model->get_data_by_encryptId('sms_exam_grade_scale',$editid);
		else:
		...
		if($this->input->post('SaveChanges')):
			...
			if($this->form_validation->run() && $error == 'NO'):   
				...
				$param['value']  		= 	addslashes($this->input->post('value'));
				$param['lower_mark']	= 	addslashes($this->input->post('lower_mark'));
				$param['upper_mark']	= 	addslashes($this->input->post('upper_mark'));	
				if($this->input->post('CurrentDataID') ==''):
					...
					$param['status']			=	'Y';
				   	$alastInsertId				=	$this->common_model->add_data('sms_exam_grade_scale',$param);					
										
					$Uparam['encrypt_id']		=	manojEncript($alastInsertId);
					$Uwhere['id']	=	$alastInsertId;
					$this->common_model->edit_data_by_multiple_cond('sms_exam_grade_scale',$Uparam,$Uwhere);
					$this->session->set_flashdata('alert_success',lang('addsuccess'));
				else:
					$subjectHeadId				=	$this->input->post('CurrentDataID');
					$param['update_date']		=	currentDateTime();
					$param['updated_by']		=	$this->session->userdata('SMS_ADMIN_ID');
					$this->common_model->edit_data('sms_exam_grade_scale',$param,$subjectHeadId);
		...
		$this->layouts->admin_view('gradescale/addeditdata',array(),$data);
	}	// END OF FUNCTION	
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->edit_data('sms_exam_grade_scale',$param,$changeStatusId);
