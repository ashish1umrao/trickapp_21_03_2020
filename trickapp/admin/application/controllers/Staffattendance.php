...
class staffattendance extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->helper('language','MY_url_helper');
    ...
    public  function index() {
        ...
        ///all teachers name ////
        $teacherQuery      = "SELECT encrypt_id, user_f_name, user_m_name, user_l_name, user_mobile FROM sms_users WHERE franchise_id = '" . $this->session->userdata('SMS_ADMIN_FRANCHISE_ID') . "' 
							  AND school_id = '" . $this->session->userdata('SMS_ADMIN_SCHOOL_ID') . "'
							  AND branch_id = '" . $this->session->userdata('SMS_ADMIN_BRANCH_ID') . "'AND user_type!='Teacher' AND user_type!='Parent'  AND status='A' ";
        //echo "<pre>"; print_r($teacherQuery); die;
        $data['TEACHERDATA'] = $this->common_model->get_data_by_query('multiple', $teacherQuery);
        //echo "<pre>"; print_r($data['TEACHERDATA']); die;
        $data['error']       = '';
        ...
        if ($staffid != 'All') :
            //single techer query
            $singlestaffQuery = "SELECT user_f_name ,user_m_name ,user_l_name ,user_mobile ,employee_id  FROM sms_users WHERE encrypt_id = '" . $staffid . "'  AND franchise_id = '" . $this->session->userdata('SMS_ADMIN_FRANCHISE_ID') . "' 
								AND school_id = '" . $this->session->userdata('SMS_ADMIN_SCHOOL_ID') . "'
								AND branch_id = '" . $this->session->userdata('SMS_ADMIN_BRANCH_ID') . "'AND user_type!='Teacher' AND user_type!='Parent'   AND status='A' ";
            $staffInfo = $this->common_model->get_data_by_query('single', $singlestaffQuery);
            //$time             = addTimeInDate();
            ...
            //echo "<pre>"; print_r($startDate); echo "</br>";
            //echo "<pre>"; print_r($endDate); die;
            
            //find joining date
            $joiningDateQuery = "SELECT user_joining_date   FROM `sms_user_detils` WHERE user_id = '" . $staffid . "' ";
            //
            $joining_date = $this->common_model->get_data_by_query('single', $joiningDateQuery);
            $joiningDate  = ($joining_date['user_joining_date']);
            
            if($staffInfo):
                if ($joiningDate < $endDate) :
                    //all present data
                    $alldataQuery    = "SELECT attnd.date ,  attnd.time ,t.user_f_name,t.user_l_name,t.user_m_name, t.employee_id , t.user_mobile,attnd.attendance FROM `sms_staff_attendance` AS attnd
                                        LEFT JOIN `sms_users` AS t  ON t.encrypt_id = attnd.staff_id WHERE attnd.staff_id ='" . $staffid . "'  AND (attnd.date) >= '" . ($startDate) . "' AND (attnd.date)  <= '" . ($endDate) . "'
                                        ";
                    ...
                    //     //     $tmpArry                = array();
                    //     //     $tmpArry                = '';
                    //     //     $tmpArry['user_f_name'] = $staffInfo['user_f_name'];
                    //     //     $tmpArry['user_m_name'] = $$staffInfo['user_m_name'];
                    //     //     $tmpArry['user_l_name'] = $staffInfo['user_l_name'];
                    //     //     $tmpArry['user_mobile'] = $staffInfo['user_mobile'];
                    //     //     $tmpArry['employee_id'] = $staffInfo['employee_id'];
            ...
            $startDate       = addTimeInDate(DDMMYYtoYYMMDD($this->input->get('date')));
            $endDate         = addTimeInDate((DDMMYYtoYYMMDD($this->input->get('date'))));
            $alldataQuery    = "SELECT attnd.date , attnd.staff_id , attnd.time ,t.user_f_name,t.user_l_name,t.user_m_name, t.employee_id ,t.user_mobile,attnd.attendance FROM `sms_staff_attendance` AS attnd
                                LEFT JOIN `sms_users` AS t  ON t.encrypt_id = attnd.staff_id     LEFT JOIN sms_user_detils AS d  ON  d.user_id = t.encrypt_id WHERE  (attnd.date) >= '" . ($startDate) . "' AND (attnd.date)  <= '" . ($endDate) . "'
            ...
            //echo "<pre>"; print_r($alldataQuery); die;
            $allPresentAttnd = $this->common_model->get_data_by_query('multiple', $alldataQuery);
            $staffQuery = "SELECT t.user_f_name,t.user_l_name,t.user_m_name,t.user_mobile ,t.employee_id, t.encrypt_id FROM `sms_users` AS t
        ...
        if ($this->input->post('report')):
            if($staffid):
                $this->downloadStaffAttendance($data['ALLDATA']);
                //excel download end 
            else:
        ...
        if ($this->input->post('SaveTeacherExcelUpload')):
            $this->addStaffAttendanceByExcel($data,$_FILES);
        endif;
        // end Upload excel
        ...
        $this->layouts->admin_view('staffattendance/index', array(), $data);
    } // END OF FUNCTION

    /* * *********************************************************************
     * * Function name : downloadStaffAttendance
     * * Developed By : Manoj Kumar
     * * Purpose  : This function used for Download Staff Attendance
     * * Date : 11 DECEMBER 2018
     * * ********************************************************************* */
    public function downloadStaffAttendance($ALLDATA) {
        $tmpReportArry = array();
        $reportData    = array();
        ...
        $filename = 'Staff_Attendance_Report.xls';
    ...
    /* * *********************************************************************
     * * Function name : addStaffAttendanceByExcel
     * * Developed By : Manoj Kumar
     * * Purpose  : This function used for add Staff Attendance By Excel
     * * Date : 11 DECEMBER 2018
     * * ********************************************************************* */
    public function addStaffAttendanceByExcel($data,$FILES) {
        ...
        $pathinfo    = pathinfo($_FILES['staffFile']['name']);
        ...
        if ($file_extension == 'csv'):
            ...
            $tmpname  = $_FILES['staffFile']['tmp_name'];
                ...
                if ($handle = fopen($dir . $filename, "r")):
                    while (($csvData = fgetcsv($handle, 10000)) !== FALSE):
                        ...
                        if ($row > 1):
                            ...
                            $attendance   = $branddataarray[6];
                            
                            $data1 = array();
                            foreach ($teacherData as $TD): //echo "<pre>"; print_r($TD); die;
                                
                        $staffattendQuery = "SELECT st.staff_attendance_id, st.staff_id 
                                FROM `sms_staff_attendance` AS st 
                                WHERE   st.staff_id = '".$TD['encrypt_id']."'
                                AND st.date = '".$year . '-' . $month . '-' . $date."'";
                        $studentattendData  = $this->common_model->get_data_by_query('single', $staffattendQuery);  
