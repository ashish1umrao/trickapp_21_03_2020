...
class Studentlist extends CI_Controller {
    ...
    public function index() {
        ...
        if ($this->input->get('searchValue')):
            ...
            $whereCon['like']    = "( stu.student_qunique_id LIKE '%" . $sValue . "%' OR  stubr.student_registration_no LIKE '%" . $sValue . "%' 
                                      ...
                                      OR per.user_email LIKE '%" . $sValue . "%' 
                                      OR per.user_mobile LIKE '%" . $sValue . "%' 
        ...
        $shortField = 'stubr.student_branch_id DESC';
    ...
    public function addeditdata($editid = '') {
        ...
        $data['MULTIPLECLASSDATA']      = $this->common_model->get_data_by_query('multiple', $subQuery);
        ...
        if ($editid):
            ...
            $data['ALLSECTIONDATA'] 		= 	$this->admin_model->get_all_section_data_byid($editid);
            $stubrQuery                 = "SELECT student_registration_no,student_admission_date,student_relieving_date FROM sms_student_branch WHERE student_qunique_id = '" . $editid . "'";
    ...
    /*===============================Multiple Student Edit Data ====================================*/
          
           $stusibstuQuery                 = 	"SELECT ss.*,ssc.*,ssb.* FROM sms_sibling_students as ss LEFT JOIN sms_sibling_student_class as ssc ON ss.student_qunique_id = ssc.student_sibling_id LEFT JOIN sms_sibling_students_branch as ssb ON ssb.student_sibling_id = ssc.student_sibling_id WHERE ss.student_parent_id = '" . $editid . "'";
		   //echo "<pre>"; print_r($stusibstuQuery); die;
		   $data['SIBEDITDATA']    		   =    $this->common_model->get_data_by_query('multiple', $stusibstuQuery);
		 //echo "<pre>"; print_r($data['SIBEDITDATA']); die;
      ...
      /*====================================End==================================================*/
		  ...
		  if ($this->input->post('SaveChanges')): //echo "<pre>"; print_r($_POST); die;
				$error = 'NO';
			$this->form_validation->set_rules('student_child_id', 'Student Child ID', 'trim');
            ...
            $this->form_validation->set_rules('student_roll_no', 'Roll no', 'trim');
            ...
            $this->form_validation->set_rules('uploadimage0', 'Profile picture', 'trim');
                ...
                /*------- Multiple Student Start ----------------------*/
                $TotalmoredetailsCount     =  $this->input->post('TotalmoredetailsCount');
                //echo "<pre>"; print_r($TotalmoredetailsCount); die;
                if($TotalmoredetailsCount):  
                    for($i=1; $i <= $TotalmoredetailsCount; $i++):
                         //$this->form_validation->set_rules('class_id_'.$i, 'Class name');
                         //$this->form_validation->set_rules('section_id_'.$i, 'Section name', 'trim');
                         $this->form_validation->set_rules('student_roll_no_'.$i, 'Roll no', 'trim');
                         $this->form_validation->set_rules('student_registration_no_'.$i, 'Registration no', 'trim');
                         $this->form_validation->set_rules('student_admission_date_'.$i, 'Admission date', 'trim');
                         $this->form_validation->set_rules('student_relieving_date_'.$i, 'Relieving date', 'trim');
                         $this->form_validation->set_rules('student_f_name_'.$i, 'First name', 'trim|min_length[2]|max_length[50]');
                         $this->form_validation->set_rules('student_m_name_'.$i, 'Middle name', 'trim');
                         $this->form_validation->set_rules('student_l_name_'.$i, 'Last name', 'trim');
                         $this->form_validation->set_rules('student_dob_'.$i, 'Date of birth', 'trim');
                         $this->form_validation->set_rules('student_gender_'.$i, 'Gender', 'trim');
                         $this->form_validation->set_rules('student_religion_'.$i, 'Religion', 'trim');
                         $this->form_validation->set_rules('student_category_'.$i, 'Category', 'trim');
                         $this->form_validation->set_rules('student_visible_mark_'.$i, 'Visible mark', 'trim');
                        // $this->form_validation->set_rules('uploadimage0_'.$i, 'Profile picture', 'trim');
                    endfor;
                    //$this->form_validation->set_rules('TotalmoredetailsCount', 'Total Question', 'trim');
                endif;
        ...
        /*********************************************************************************************/
                if ($this->input->post('CurrentDataID') == ''):  
					...
					$TotalmoredetailsCount     	  =  $this->input->post('TotalmoredetailsCountData');
                    //echo "<pre>"; print_r($TotalmoredetailsCount); die;
                    if($TotalmoredetailsCount):  
						for($i=1; $i <= $TotalmoredetailsCount; $i++):
					//$StudentChildID     		  =  $this->input->post('student_child_id');
                         //////////////	STUDENT SCHOOL BRANCH RELATION 	///////////////////////
				$SSBParam['student_registration_no'] = addslashes($this->input->post('student_registration_no_'.$i));
				$SSBParam['student_admission_date']  = DDMMYYtoYYMMDD($this->input->post('student_admission_date_'.$i));
				$SSBParam['student_relieving_date']  = DDMMYYtoYYMMDD($this->input->post('student_relieving_date_'.$i));
				//////////////	STUDENT CLASS SECTION RELATION 	///////////////////////
                $SSCSParam['class_id']        		= addslashes($this->input->post('class_id_'.$i));
                $SSCSParam['section_id']      		= addslashes($this->input->post('section_id_'.$i));
                $SSCSParam['student_roll_no'] 		= addslashes($this->input->post('student_roll_no_'.$i));
                //////////////	STUDENT SECTION 	///////////////////////
                $SSParam['student_f_name']       = addslashes($this->input->post('student_f_name_'.$i));
                $SSParam['student_m_name']       = addslashes($this->input->post('student_m_name_'.$i));
                $SSParam['student_l_name']       = addslashes($this->input->post('student_l_name_'.$i));
                $SSParam['student_dob']          = DDMMYYtoYYMMDD($this->input->post('student_dob_'.$i));
                $student_gender                  = $this->input->post('student_gender_'.$i) ? explode('___', $this->input->post('student_gender_'.$i)) : array();
                $SSParam['student_gender']       = $student_gender[0] ? $student_gender[0] : '';
                $SSParam['student_gender_short'] = $student_gender[1] ? $student_gender[1] : '';
                $SSParam['student_religion']     = addslashes($this->input->post('student_religion_'.$i));
                $SSParam['student_category']     = addslashes($this->input->post('student_category_'.$i));
                $SSParam['student_visible_mark'] = addslashes($this->input->post('student_visible_mark_'.$i));
                $SSParam['student_image']        = addslashes($this->input->post('uploadimage0_'.$i));
                 //echo "<pre>";print_r($SSCSParam); '</hr>'; print_r($SSBParam);'</hr>'; print_r($SSParam);die;
					
					/*****************************END *********************************** */
                     $SSParam['creation_date']   = currentDateTime();
                     $SSParam['created_by']      = $this->session->userdata('SMS_ADMIN_ID');
                     $SSParam['status']          = 'A';
                     $SSParam['session_year']	 =	CURRENT_SESSION;
                     $SSParam['student_parent_id'] = $studentUniqueId;
                     $stuSIBlastInsertId         = $this->common_model->add_data('sibling_students', $SSParam);
                     $SSPUparam['encrypt_id']         = manojEncript($stuSIBlastInsertId);
                     $SSPUparam['student_qunique_id'] = studentUniqueId($SSParam['student_dob'], $SSParam['student_f_name']);
                    $SSPUwhere['id']         = $stuSIBlastInsertId;
                    $this->common_model->edit_data_by_multiple_cond('sibling_students', $SSPUparam, $SSPUwhere);
                    if ($studentUniqueId && $stuSIBlastInsertId):
                        //////////////	STUDENT SCHOOL BRANCH RELATION 	///////////////////////
                        $SSBParam['franchise_id']       = $this->session->userdata('SMS_ADMIN_FRANCHISE_ID');
                        $SSBParam['school_id']          = $this->session->userdata('SMS_ADMIN_SCHOOL_ID');
                        $SSBParam['branch_id']          = $this->session->userdata('SMS_ADMIN_BRANCH_ID');
                        $SSBParam['board_id']           = $this->session->userdata('SMS_ADMIN_BOARD_ID');
                        $SSBParam['student_qunique_id'] = $studentUniqueId;
                        $SSBParam['student_sibling_id'] = $SSPUparam['student_qunique_id'];
                        $SSBParam['creation_date']      = currentDateTime();
                        $SSBParam['created_by']         = $this->session->userdata('SMS_ADMIN_ID');
                        $SSBParam['session_year']			=	CURRENT_SESSION;
                        $SSBParam['status']             = 'Y';
                        $stubrLastInsertId             = $this->common_model->add_data('sibling_students_branch',$SSBParam);
                        $SSBUparam['encrypt_id']        = manojEncript($stubrLastInsertId);
                        $SSBUwhere['student_branch_id'] = $stubrLastInsertId;
                        $this->common_model->edit_data_by_multiple_cond('sibling_students_branch', $SSBUparam, $SSBUwhere);

                        //////////////	STUDENT CLASS SECTION RELATION 	///////////////////////
                        $SSCSParam['franchise_id']       = $this->session->userdata('SMS_ADMIN_FRANCHISE_ID');
                        $SSCSParam['school_id']          = $this->session->userdata('SMS_ADMIN_SCHOOL_ID');
                        $SSCSParam['branch_id']          = $this->session->userdata('SMS_ADMIN_BRANCH_ID');
                        $SSCSParam['board_id']           = $this->session->userdata('SMS_ADMIN_BOARD_ID');
                        $SSCSParam['student_qunique_id'] = $studentUniqueId;
                        $SSCSParam['student_sibling_id'] = $SSPUparam['student_qunique_id'];
                        $SSCSParam['current_year']       = currentDateTime();
                        $SSCSParam['creation_date']      = currentDateTime();
                        $SSCSParam['created_by']         = $this->session->userdata('SMS_ADMIN_ID');
                        $SSCSParam['status']             = 'Y';
                        $SSCSParam['session_year']			=	CURRENT_SESSION;
                        $stuclsLastInsertId             = $this->common_model->add_data('sms_sibling_student_class', $SSCSParam);
                        $SSCSUparam['encrypt_id']       = manojEncript($stuclsLastInsertId);
                        $SSCSUwhere['student_class_id'] = $stuclsLastInsertId;
                        $this->common_model->edit_data_by_multiple_cond('sibling_student_class', $SSCSUparam, $SSCSUwhere);
						endif;
					endfor;
				endif;
                          /*****************End**************************/
                    ...
                    $this->common_model->edit_data_by_multiple_cond('student_class', $SCSParam, $SCSUwhere);
						...
						$TotalmoredetailsCount     		=  $this->input->post('TotalmoredetailsCount');
						$StudentChildID     		    =  $this->input->post('student_id');
						if($TotalmoredetailsCount):  
							for($i=1; $i <= $TotalmoredetailsCount; $i++):
						$StudentSIBChildID     		  =  $this->input->post('student_child_id_'.$i);
						//////////////	STUDENT SCHOOL BRANCH RELATION 	///////////////////////
						$SSBParam['student_registration_no'] = addslashes($this->input->post('student_registration_no_'.$i));
						$SSBParam['student_admission_date']  = DDMMYYtoYYMMDD($this->input->post('student_admission_date_'.$i));
						$SSBParam['student_relieving_date']  = DDMMYYtoYYMMDD($this->input->post('student_relieving_date_'.$i));
					  ...
					  $SSParam['student_l_name']       = addslashes($this->input->post('student_l_name_'.$i));
					  $SSParam['student_dob']          = DDMMYYtoYYMMDD($this->input->post('student_dob_'.$i));
					  $student_genderA                = $this->input->post('student_gender_'.$i) ? explode('___', $this->input->post('student_gender_'.$i)) : array();
					  $SSParam['student_gender']       = $student_genderA[0] ? $student_genderA[0] : '';
					  $SSParam['student_gender_short'] = $student_genderA[1] ? $student_genderA[1] : '';
					  $SSParam['student_religion']     = addslashes($this->input->post('student_religion_'.$i));
					  $SSParam['student_category']     = addslashes($this->input->post('student_category_'.$i));
					  $SSParam['student_visible_mark'] = addslashes($this->input->post('student_visible_mark_'.$i));
					  $SSParam['student_image']        = addslashes($this->input->post('uploadimage0_'.$i));

                      //echo "<pre>"; print_r($SSParam); echo "</br>"; die;
								//////////////	STUDENT SIBLING CLASS SECTION RELATION 	///////////////////////
						$SSParam['update_date']          = currentDateTime();
						$SSParam['updated_by']           = $this->session->userdata('SMS_ADMIN_ID');
						$SSCSUwhere['student_qunique_id'] = $StudentSIBChildID;
						$SSCSUwhere['student_parent_id'] = $StudentChildID;
						$this->common_model->sibling_edit_data_by_multiple_cond('sms_sibling_students', $SSParam, $SSCSUwhere);

						//////////////	STUDENT Sibling SCHOOL BRANCH RELATION 	///////////////////////
						$SSCSParam['update_date']          = currentDateTime();
						$SSCSParam['updated_by']           = $this->session->userdata('SMS_ADMIN_ID');
						$SSBUwhere['student_sibling_id'] = $StudentSIBChildID;
					    $SSBUwhere['student_qunique_id'] = $StudentChildID;
						$this->common_model->sibling_edit_data_by_multiple_cond('sibling_student_class', $SSCSParam, $SSBUwhere);
					  
					  //////////////	STUDENT Sibling SCHOOL BRANCH RELATION 	///////////////////////
						$SSBParam['update_date']          = currentDateTime();
						$SSBParam['updated_by']           = $this->session->userdata('SMS_ADMIN_ID');
						$SSBUUwhere['student_sibling_id'] = $StudentSIBChildID;
					    $SSBUUwhere['student_qunique_id'] = $StudentChildID;
						$this->common_model->sibling_edit_data_by_multiple_cond('sms_sibling_students_branch', $SSBParam, $SSBUUwhere);
					  endfor;
				 endif;
						/****************************************END******************************************* */
    ...
    public function addeditparents($studentId = '') {
        ...
        if ($this->input->post('SaveChanges1'))://echo "<pre>"; print_r($_POST); die;
            $error = 'NO';
