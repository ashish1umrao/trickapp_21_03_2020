...
class Adminfeedbackrequest extends CI_Controller {
	...
	{	
		...
		if($this->input->get('searchValue')):
			...
			$whereCon['like']		 		= 	"(fback.sender_type LIKE '%".$sValue."%'
												  )";
		...
		$whereCon['where']		 			= 	"fback.sender_type = 'Teacher' AND fback.franchise_id = '".$this->session->userdata('SMS_ADMIN_FRANCHISE_ID')."' AND fback.school_id = '".$this->session->userdata('SMS_ADMIN_SCHOOL_ID')."' AND fback.branch_id = '".$this->session->userdata('SMS_ADMIN_BRANCH_ID')."'";		
		$shortField 						= 	'fback.feedback_id ASC';
		
		$this->load->library('pagination');
		$config['base_url'] 				= 	$this->session->userdata('SMS_ADMIN_PATH').$this->router->fetch_class().'/index';
		$this->session->set_userdata('feedbackrequestAdminData',currentFullUrl());
		$qStringdata						=	explode('?',currentFullUrl());
		$config['suffix']					= 	$qStringdata[1]?'?'.$qStringdata[1]:'';
		$tblName 							= 	'feedback as fback';
		$con 								= 	'';
		$config['total_rows'] 				= 	$this->admin_model->Selectfeedbackrequest('count',$tblName,$whereCon,$shortField,'0','0');
       ...
       if ($this->uri->segment(getUrlSegment())):
		...
		$data['ALLDATA'] 					= 	$this->admin_model->Selectfeedbackrequest('data',$tblName,$whereCon,$shortField,$config['per_page'],$page); 
		//echo "<pre>"; print_r($data['ALLDATA']); die;
		$this->layouts->set_title('Manage Feedback Request');
		$this->layouts->admin_view('adminfeedbackrequest/index',array(),$data);
	}	// END OF FUNCTION
	...
	{		
		...
		if($this->input->post('SaveChanges')): 
			...
			if($this->form_validation->run() && $error == 'NO'):   
				...
				redirect(correctLink('feedbackrequestAdminData',$this->session->userdata('SMS_ADMIN_PATH').$this->router->fetch_class().'/index'));
			endif;
		endif;
		
		$this->layouts->set_title('Edit Teacher leave Request');
		$this->layouts->admin_view('teacherleaverequest/addeditdata',array(),$data);
	}	// END OF FUNCTION	
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->edit_data('feedback',$param,$changeStatusId);
