...
class Teacherleaverequest extends CI_Controller {
	...
	{	
		...
		$whereCon['where']		 			= 	"lev.user_type = 'Teacher' AND lev.franchise_id = '".$this->session->userdata('SMS_ADMIN_FRANCHISE_ID')."' AND lev.school_id = '".$this->session->userdata('SMS_ADMIN_SCHOOL_ID')."' AND lev.branch_id = '".$this->session->userdata('SMS_ADMIN_BRANCH_ID')."'";		
		$shortField 						= 	'lev.leave_id ASC';
		
		$this->load->library('pagination');
		$config['base_url'] 				= 	$this->session->userdata('SMS_ADMIN_PATH').$this->router->fetch_class().'/index';
		$this->session->set_userdata('teacherleaverequestAdminData',currentFullUrl());
		$qStringdata						=	explode('?',currentFullUrl());
		$config['suffix']					= 	$qStringdata[1]?'?'.$qStringdata[1]:'';
		$tblName 							= 	'leave as lev';
		$con 								= 	'';
		$config['total_rows'] 				= 	$this->admin_model->SelectteacherLeaverequest('count',$tblName,$whereCon,$shortField,'0','0');
       ...
       if ($this->uri->segment(getUrlSegment())):
		...
		$data['ALLDATA'] 					= 	$this->admin_model->SelectteacherLeaverequest('data',$tblName,$whereCon,$shortField,$config['per_page'],$page); 
		//echo "<pre>"; print_r($data['ALLDATA']); die;
		$this->layouts->set_title('Manage Teacher leave Request');
		$this->layouts->admin_view('teacherleaverequest/index',array(),$data);
	}	// END OF FUNCTION
	...
	{		
		...
		if($this->input->post('SaveChanges')): 
			...
			if($this->form_validation->run() && $error == 'NO'):   
				...
				redirect(correctLink('teacherleaverequestAdminData',$this->session->userdata('SMS_ADMIN_PATH').$this->router->fetch_class().'/index'));
			endif;
		endif;
