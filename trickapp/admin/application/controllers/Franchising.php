...
class Franchising extends CI_Controller {
	...
	{	
       ...
       if ($this->uri->segment(getUrlSegment())):
		...
		$this->layouts->set_title('Manage Franchisee details');
		$this->layouts->admin_view('franchising/index',array(),$data);
	}	// END OF FUNCTION
		...
		if($this->input->post('SaveChanges')):
			...
			if($this->form_validation->run() && $error == 'NO'):  
				...
				if($this->input->post('CurrentDataID') ==''):
					...
					$franchisingId				=	$this->input->post('CurrentDataID');
					$param['update_ip']			=	currentIp();
					$param['update_date']		=	currentDateTime();
					$param['updated_by']		=	$this->session->userdata('SMS_ADMIN_ID');
					$this->common_model->edit_data('admin',$param,$franchisingId);
					$this->session->set_flashdata('alert_success',lang('updatesuccess'));
				endif;
				
				redirect(correctLink('franchisingAdminData',$this->session->userdata('SMS_ADMIN_PATH').$this->router->fetch_class().'/index'));
			endif;
		endif;
		
		$this->layouts->set_title('Edit Franchisee details');
		$this->layouts->admin_view('franchising/addeditdata',array(),$data);
	}	// END OF FUNCTION	
	...
	function get_view_data()
	{
		...
		if($this->input->post('viewid')):
			...
			$viewQuery			=	"SELECT adm.*
									 ...
									 WHERE adm.encrypt_id = '".$viewId."' AND adm.admin_type = 'Franchising'";
