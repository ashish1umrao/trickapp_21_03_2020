...
class Teachattendance extends CI_Controller {
    ...
    public
        ...
        if ($teacherid != 'All') :
            ...
            $singleTeacherQuery = "SELECT user_f_name ,user_m_name ,user_l_name ,user_mobile ,employee_id  FROM sms_users WHERE encrypt_id = '" . $teacherid . "'  
            ...
            if ($joiningDate < $endDate) :
                ...
                //all present data
                $alldataQuery    = "SELECT attnd.date ,  attnd.time ,t.user_f_name,t.user_l_name,t.user_m_name, t.employee_id , t.user_mobile FROM `sms_teacher_attendance` AS attnd
LEFT JOIN `sms_users` AS t  ON t.encrypt_id = attnd.teacher_id WHERE attnd.teacher_id ='" . $teacherid . "'  AND UNIX_TIMESTAMP(attnd.date) >= '" . ($startDate) . "' AND UNIX_TIMESTAMP(attnd.date)  <= '" . ($endDate) . "'
 AND attnd.franchise_id = '" . $this->session->userdata('SMS_ADMIN_FRANCHISE_ID') . "' 
            ...
            $alldataQuery    = "SELECT attnd.date , attnd.teacher_id , attnd.time ,t.user_f_name,t.user_l_name,t.user_m_name, t.employee_id ,t.user_mobile FROM `sms_teacher_attendance` AS attnd
LEFT JOIN `sms_users` AS t  ON t.encrypt_id = attnd.teacher_id     LEFT JOIN sms_user_detils AS d  ON  d.user_id = t.encrypt_id WHERE  UNIX_TIMESTAMP(attnd.date) >= '" . ($startDate) . "' AND UNIX_TIMESTAMP(attnd.date)  <= '" . ($endDate) . "'
            ...
            foreach ($allteacher as $AT):
                ...
                if ($allPresentAttnd):
                    foreach ($allPresentAttnd as $APA):
                        ...
                        if ($APA['teacher_id'] == $AT['encrypt_id']):
                            $encryptFlag       = '1';
                            $APA['attandance'] = 'Present';
...
if($teacherid):
            ...
            $this->excel->getActiveSheet()->getStyle('A1:H1')->getFill()->applyFromArray(array(
            ...
            $this->excel->getActiveSheet()->getStyle('A1:F1')->getAlignment()->setTextRotation(90);
                            ...
                            if ($row > 1):
                                ...
                                foreach ($teacherData as $TD):
                                    ...
                                    if ($TD['user_mobile'] == $mobile):
                                        $data1['teacher_id']   = $TD['encrypt_id'];
                                        ...
                                        $ulastInsertId          = $this->common_model->add_data('teacher_attendance', $data1);
                                        $TUparam['encrypt_id']            = manojEncript($ulastInsertId);
                                        $TUwhere['teacher_attendance_id'] = $ulastInsertId;
                                        $this->common_model->edit_data_by_multiple_cond('teacher_attendance', $TUparam, $TUwhere);
