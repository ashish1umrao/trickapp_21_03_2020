...
class Admin_model extends CI_Model
{
	...
	{ 	//echo $userType.'---'.$showType; die;
		if($this->session->userdata('SMS_ADMIN_ID') == ""):
			setcookie('SMS_ADMIN_REFERENCEPAGES', uri_string(), time() + 60*60*24*5, '/');
			redirect(base_url());
		else:
			$urlError 			=	'No';
			if(!strpos(currentFullUrl(),str_replace(base_url(),'',$this->session->userdata('SMS_ADMIN_PATH')))):
				$urlError 		=	'Yes';
			endif;
			
			if($userType == "admin"):	
				if($showType == '' && $urlError == 'No'):
					return true;
				elseif($this->checkPermission($showType) && $urlError == 'No'):  
					return true;
				else:		
	...
	************************************************************************/
	...
	{  //echo  $showType; die;
		...
		if($this->session->userdata('SMS_ADMIN_TYPE') == 'Super admin'):
			...
			if($this->router->fetch_class() == 'adminsmsusers'):
				return true;
			elseif($showType == 'view_data'):
				return true;
			else:
	...
	************************************************************************/
	...
	{ 
		...
		$this->db->join('admin as badm','adm.admin_branch_id=badm.encrypt_id','LEFT');
		$this->db->join('department as dep','adm.admin_department_id=dep.encrypt_id','LEFT');
                $this->db->join('designation as deg','adm.admin_designation_id=deg.encrypt_id','LEFT');
	...
	************************************************************************/
	function get_board_for_navigation()
	{
		...
		$this->db->select('board_id,branch_board_name');
	...
	************************************************************************/
	...
	{ 
		...
		$this->db->from($tblName);
		$this->db->join('admin as badm','deg.branch_id=badm.encrypt_id','LEFT');
		$this->db->join('admin as sadm','deg.school_id=sadm.encrypt_id','LEFT');
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
		if($whereCon['like']):  $this->db->where($whereCon['like']); 		endif;
		if($shortField):		$this->db->order_by($shortField);			endif;
		if($numPage):			$this->db->limit($numPage,$cnt);			endif;
	...
	************************************************************************/
	...
	{ 
		$this->db->select('stubr.student_registration_no, stubr.student_admission_date, stubr.student_relieving_date, 
						   stu.*, cls.class_name, clsec.class_section_name,stucls.class_id,stucls.section_id,per.user_email,per.user_mobile');
		$this->db->from($tblName);
        ...
        if ($edit_id):
            ...
            if ($viewData <> ""):
                ...
                $ustuclsQuery  = "SELECT cls.class_name, clsec.class_section_name, stucls.student_roll_no
											 ...
											 LEFT JOIN sms_sibling_student_class as sbs  ON stucls.student_qunique_id=sbs.student_qunique_id
                ...
                //echo "<pre>"; print_r($ustuclsQuery); die;
...
{ 
$this->db->select('fhead.*, fheading.heading_type');
$this->db->from($tblName);
...
{ 
$this->db->select('fhead.fee_head_name, ffine.*,fheading.heading_type');
$this->db->from($tblName);
$this->db->join('fee_head as fhead','fhead.encrypt_id=ffine.fee_head_id','LEFT');
$this->db->join('fee_heading as fheading','ffine.fee_head_id=fheading.encrypt_id','LEFT');
if($whereCon['where']):	$this->db->where($whereCon['where']); endif;
if($whereCon['like']):  $this->db->where($whereCon['like']); endif;
if($shortField):	$this->db->order_by($shortField);	endif;
if($numPage):	$this->db->limit($numPage,$cnt);	endif;
...
{ 
$this->db->select('con.*,fheading.heading_type ');
$this->db->from($tblName);
$this->db->join('fee_heading as fheading','ffine.fee_head_id=fheading.encrypt_id','LEFT');
if($whereCon['where']):	$this->db->where($whereCon['where']); endif;
if($whereCon['like']):  $this->db->where($whereCon['like']); endif;
...
/***********************************************************************
	** Function name : SelectEventData
	** Developed By : Manoj Kumar
	** Purpose  : This function used for select teacher development program data
	** Date : 23 JANUARY 2018
	************************************************************************/
	function SelectEventData($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
		$this->db->select('ev.*');
		$this->db->from($tblName);
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
...
/***********************************************************************
	** Function name : SelectHolidayData
	** Developed By : Manoj Kumar
	** Purpose  : This function used for select teacher development program data
	** Date : 23 JANUARY 2018
	************************************************************************/
	function SelectHolidayData($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
		$this->db->select('h.*');
		$this->db->from($tblName);
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
    ...
    /***********************************************************************
	** Function name : SelecthostelListData
	** Developed By : Manoj Kumar
	** Purpose  : This function used for Select Fee Head List Data
	** Date : 06 APRIL 2018
	************************************************************************/
	function SelecthostelListData($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
		$this->db->select('w.user_f_name,w.user_m_name,w.user_l_name, h.*');
		$this->db->from($tblName);
		$this->db->join('users as w','w.encrypt_id=h.hostel_warden_id','LEFT');
		if($whereCon['where']):	$this->db->where($whereCon['where']); endif;
		if($whereCon['like']):  $this->db->where($whereCon['like']); endif;
		if($shortField):	$this->db->order_by($shortField);	endif;
		if($numPage):	$this->db->limit($numPage,$cnt);	endif;
	...
	/***********************************************************************
	** Function name : SelectAppDashboardData
	** Developed By : Manoj Kumar
	** Purpose  : This function used for Select App Dashboard Data
	** Date : 12 DECEMBER 2018 
	************************************************************************/
	function SelectAppDashboardData($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
		$this->db->select('add.*,');
		$this->db->from($tblName);
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
	...
	/***********************************************************************
	** Function name : SelectGalleryData
	** Developed By : Manoj Kumar
	** Purpose  : This function used for Select Gallery Data
	** Date : 14 DECEMBER 2018
	************************************************************************/
	function SelectGalleryData($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
		$this->db->select('gall.*, galim.image_name');
		$this->db->from($tblName);
		$this->db->join('gallery_image as galim','gall.encrypt_id=galim.gallery_id','LEFT');
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
		if($whereCon['like']):  $this->db->where($whereCon['like']); 		endif;
		if($shortField):		$this->db->order_by($shortField);			endif;
		$this->db->group_by("galim.gallery_id");
		if($numPage):			$this->db->limit($numPage,$cnt);			endif;
		$query = $this->db->get();
		if($action == 'data'):
			if($query->num_rows() > 0):
	...
	/***********************************************************************
	** Function name : SelectNewsData
	** Developed By : Manoj Kumar
	** Purpose  : This function used for Select News Data
	** Date : 14 DECEMBER 2018
	************************************************************************/
	function SelectNewsData($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
		$this->db->select('news.*');
		$this->db->from($tblName);
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
	 ...
	 /***********************************************************************
	** Function name : SelectLeaveTypeData
	** Developed By : Manoj Kumar
	** Purpose  : This function used for Select Leave Type Data
	** Date : 17 DECEMBER 2018
	************************************************************************/
	function SelectLeaveTypeData($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
		$this->db->select('levty.*');
		$this->db->from($tblName);
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
		if($whereCon['like']):  $this->db->where($whereCon['like']); 		endif;
		if($shortField):		$this->db->order_by($shortField);			endif;
...
################################################ JITENDRA CODE START ########################################
	/***********************************************************************
	** Function name : SelectExamTermData
	** Developed By : Jitendra Singh
	** Purpose  : This function used for select Exam Term data
	** Date : 30 APRIL 2019
	************************************************************************/
	function SelectExamTermData($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
		$this->db->select('*');
		$this->db->from($tblName);		
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
	...
	** Date : 30 APRIL 2019
	************************************************************************/
	function SelectExamName($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
		$this->db->select('se.*,set.encrypt_id as branch_id,set.name as term_name');
		$this->db->from($tblName);
		$this->db->join('sms_exam_term as set','se.term_id=set.encrypt_id','LEFT');
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
		if($whereCon['like']):  $this->db->where($whereCon['like']); 		endif;
		if($shortField):		$this->db->order_by($shortField);			endif;
		if($numPage):			$this->db->limit($numPage,$cnt);			endif;
	...
	** Date : 30 APRIL 2019
	************************************************************************/
	function SelectAllData($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
		$this->db->select('*');
		$this->db->from($tblName);		
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
	...
	function SelectNotificationData($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
...
############################# JITENDRA CODE FINISH HERE ###################################


############################# ASHISH CODE ###########################################	
/***********************************************************************
	** Function name : SelectStudentsyallbusListData
	** Developed By : Ashish UMrao
	** Purpose  : This function used for select Student syallbus List data
	** Date : 13 JUNE 2019
	************************************************************************/
	function SelectStudentsyallbusListData($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
		$this->db->select('csyllabus.encrypt_id,csyllabus.class_id,csyllabus.subject_id,csyllabus.class_syllabus_id, csyllabus.syllabus,cls.class_name,sub.subject_name,csyllabus.status');
		$this->db->from($tblName);
		$this->db->join('subject as sub','csyllabus.subject_id=sub.encrypt_id','LEFT');
		$this->db->join('classes as cls','csyllabus.class_id=cls.encrypt_id','LEFT');
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
		if($whereCon['like']):  $this->db->where($whereCon['like']); 		endif;
		if($shortField):		$this->db->order_by($shortField);			endif;
		$this->db->group_by("csyllabus.class_syllabus_id");
		if($numPage):			$this->db->limit($numPage,$cnt);			endif;
		$query = $this->db->get();
		if($action == 'data'):
			if($query->num_rows() > 0):
...
################################## Ashish CODE START HERE ###################################	
	...
	/***********************************************************************
	** Function name : SelectSMStemplateDatashow
	** Developed By : Ashish UMrao
	** Purpose  : This function used for select Select sms templete List Data
	** Date : 14 JUNE 2019
	************************************************************************/
	function SelectSMStemplateDatashow($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 

		$this->db->select('tsms.*,csection.class_section_name,cls.class_name,sms_temp.content,sms_temp.sms_type,sms_temp.smstypename');
		$this->db->from($tblName);
		$this->db->join('class_section as csection','tsms.section_id=csection.encrypt_id','LEFT');
		$this->db->join('classes as cls','tsms.class_id=cls.encrypt_id','LEFT');
		//$this->db->join('students as s','tsms.student_id=s.encrypt_id','LEFT');
		$this->db->join('sms_sms_template as sms_temp','tsms.template_id=sms_temp.encrypt_id','LEFT');
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
		if($whereCon['like']):  $this->db->where($whereCon['like']); 		endif;
		if($shortField):		$this->db->order_by($shortField);			endif;
		$this->db->group_by("tsms.id");
		if($numPage):			$this->db->limit($numPage,$cnt);			endif;
		$query = $this->db->get();
		//echo $this->db->last_query(); die;
		if($action == 'data'):
	...
	/***********************************************************************
	** Function name : SelectClassWiseFee
	** Developed By : Ashish UMrao
	** Purpose  : This function used for Select Class Wise Fee
	** Date : 30 APRIL 2019
	************************************************************************/
	function SelectClassWiseFee($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
		$this->db->select('cwfee.*,cls.class_name,fheading.heading_type');
		$this->db->from($tblName);
		$this->db->join('classes as cls','cwfee.class_id=cls.encrypt_id','LEFT');
		$this->db->join('fee_heading as fheading','cwfee.fee_head_id=fheading.encrypt_id','LEFT');		
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
		if($whereCon['like']):  $this->db->where($whereCon['like']); 		endif;
		if($shortField):		$this->db->order_by($shortField);			endif;
		if($numPage):			$this->db->limit($numPage,$cnt);			endif;
	...
	/***********************************************************************
	** Function name : get_section_list
	** Developed By : Ashish UMrao
	** Purpose  : This function used for select Select sms templete List Data
	** Date : 14 JUNE 2019
	************************************************************************/
	function get_section_list($societyid='')
	{
		$this->db->select('id,class_section_name');
		$this->db->from('class_section');
		$this->db->where('class_id',$societyid);
		$query = $this->db->get();
		//echo $this->db->last_query(); die;
		if($query->num_rows() > 0):
			return $query->result();
	...
	/***********************************************************************
	** Function name : SelectSMStemplateData
	** Developed By : Ashish UMrao
	** Purpose  : This function used for Select SMS template Data
	** Date : 17 JUNE 2019
	************************************************************************/
	function SelectSMStemplateData($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
	 ...
	 /***********************************************************************
	** Function name : SelectstudentLeaverequest
	** Developed By : Manoj Kumar
	** Purpose  : This function used for Select student Leave request
	** Date : 19 JUNE 2019
	************************************************************************/
	function SelectstudentLeaverequest($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
		$this->db->select('lev.*,u.user_f_name,u.user_m_name,u.user_l_name,u.user_email,levt.leave_type');
		$this->db->from($tblName);
		$this->db->join('users as u','lev.user_id=u.encrypt_id','LEFT');
		$this->db->join('leave_type as levt','lev.leave_type_id=levt.encrypt_id','LEFT');
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
		if($whereCon['like']):  $this->db->where($whereCon['like']); 		endif;
		if($shortField):		$this->db->order_by($shortField);			endif;
		if($numPage):			$this->db->limit($numPage,$cnt);			endif;
		...
		elseif($action == 'count'):
		 ...
		 /***********************************************************************
	** Function name : SelectteacherLeaverequest
	** Developed By : Ashish UMrao
	** Purpose  : This function used for Select Teacher Leave request
	** Date : 19 JUNE 2019
	************************************************************************/
	function SelectteacherLeaverequest($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
		$this->db->select('lev.*,u.user_f_name,u.user_m_name,u.user_l_name,u.user_email,levt.leave_type');
		$this->db->from($tblName);
		$this->db->join('users as u','lev.user_id=u.encrypt_id','LEFT');
	...
	/***********************************************************************
	** Function name : Selectfeedbackrequest
	** Developed By : Ashish UMrao
	** Purpose  : This function used for Select Teacher Leave request
	** Date : 19 JUNE 2019
	************************************************************************/
	function Selectfeedbackrequest($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
		//$this->db->select('fback.*,u.user_f_name,u.user_m_name,u.user_l_name,u.user_email,levt.leave_type');
		$this->db->select('fback.*,s.student_f_name,cls.class_name,csection.class_section_name');
		$this->db->from($tblName);
		$this->db->join('class_section as csection','fback.to_section_id=csection.encrypt_id','LEFT');
		$this->db->join('classes as cls','fback.to_class_id=cls.encrypt_id','LEFT');
		$this->db->join('students as s','fback.to_student_id=s.student_qunique_id','LEFT');
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
		if($whereCon['like']):  $this->db->where($whereCon['like']); 		endif;
		if($shortField):		$this->db->order_by($shortField);			endif;
		if($numPage):			$this->db->limit($numPage,$cnt);			endif;
	...
	/***********************************************************************
	** Function name : get_all_fee_heading_list
	** Developed By : Ashish UMrao
	** Purpose  : This function used for select get all fee heading list
	** Date : 07 JULY 2019
	************************************************************************/
	function get_all_fee_heading_list()
	{
		$this->db->select('id,heading_type,fee_price,encrypt_id');
		$this->db->from('fee_heading');
		$query = $this->db->get();
		if($query->num_rows() > 0):
			return $query->result();
		else:
...
/***********************************************************************
** Function name : SelectstudenttotalamountListData
** Developed By : Manoj Kumar
** Purpose  : This function used for Select student total amount List Data
** Date : 06 APRIL 2018
************************************************************************/
function SelectstudenttotalamountListData($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
{ 
		$this->db->select('fhead.*, fheading.heading_type,sclasssection.class_section_name as section_name,c.class_name,s.student_f_name,fbmonth.month_name');
		$this->db->from($tblName);
		$this->db->join('fee_paybal_month as fbmonth','fhead.id=fbmonth.month_id','LEFT');
		$this->db->join('fee_heading as fheading','fhead.fee_head_name=fheading.encrypt_id','LEFT');
		...
		if($shortField):	$this->db->order_by($shortField);	endif;
		if($numPage):	$this->db->limit($numPage,$cnt);	endif;
		$this->db->group_by('fhead.student_id');
		$query = $this->db->get();
		//echo $this->db->last_query(); die;
		if($action == 'data'):
		if($query->num_rows() > 0):
		...
		/***********************************************************************
	** Function name : SelectnoticeboardListData
	** Developed By : Ashish UMrao
	** Purpose  : This function used for Select noticeboard List Data
	** Date : 21 JUNE 2019
	************************************************************************/
	function SelectnoticeboardListData($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='')
	{ 
		 $this->db->select('snoboard.*,cls.class_name,clsec.class_section_name');
		$this->db->from($tblName);
		 $this->db->join('classes as cls','snoboard.class_id=cls.encrypt_id','LEFT');
		 $this->db->join('class_section as clsec','snoboard.section_id=clsec.encrypt_id','LEFT');
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
		if($whereCon['like']):  $this->db->where($whereCon['like']); 		endif;
		if($shortField):		$this->db->order_by($shortField);			endif;
		//$this->db->group_by("stu.student_qunique_id");
		...
		/***********************************************************************
	** Function name : get_all_attedance_student_data
	** Developed By : Ashish UMrao
	** Purpose  : This function used for get all attedance student data
	** Date : 10 OCT. 2019
	************************************************************************/
	function get_all_attedance_student_data($action='',$tblName='',$whereCon='',$shortField='',$numPage='',$cnt='',$classid='',$sectionid='',$date='',$teacherid='')
	{ //echo $action.'---'.$tblName.'---'.$whereCon.'---'.$shortField.'---'.$shortField.'---'.$numPage.'---'.$cnt.'---'.$classid.'---'.$sectionid.'---'.$date.'---'.$teacherid; die;
		//$this->db->select('stdc.*,s.student_f_name,s.student_m_name,s.student_l_name,cs.class_teacher_id as teacherID,u.user_f_name ,u.user_m_name ,u.user_l_name,CONCAT(u.user_f_name,' ',u.user_m_name,' ',u.user_l_name) AS teacherName');
		$this->db->select('stdc.*,s.student_f_name,s.student_m_name,s.student_l_name,cs.class_teacher_id as teacherID,u.user_f_name ,u.user_m_name ,u.user_l_name,cs.class_section_name,sc.class_name');
		$this->db->from($tblName);
		$this->db->JOIN('students as s','s.student_qunique_id = stdc.student_qunique_id','Left');
		$this->db->JOIN('sms_class_section as cs','stdc.section_id = cs.encrypt_id','Left');
		$this->db->JOIN('sms_users as u','cs.class_teacher_id = u.encrypt_id','Left');
		$this->db->JOIN('sms_classes as sc','sc.class_id = cs.class_id','Left');
		$this->db->where('stdc.class_id',$classid);
		$this->db->where('stdc.section_id',$sectionid);
		if($whereCon['where']):	$this->db->where($whereCon['where']);	 	endif;
		if($whereCon['like']):  $this->db->where($whereCon['like']); 		endif;
		// //$this->db->group_by("stu.student_qunique_id");
		//echo $this->db->last_query(); die;
	...
	/***********************************************************************
	** Function name : get_all_section_data_byid
	** Developed By : Ashish UMrao				
	** Purpose  : This function used for get view data by id
	** Date : 02 NOVEMBER 2019
	************************************************************************/
	function get_all_section_data_byid($studentId='')
	{
		$this->db->select('sc.student_class_id,sc.student_roll_no,s.*');
		$this->db->from('student_class as sc');
		$this->db->join('students as s','sc.student_qunique_id=s.student_qunique_id','LEFT');
		$this->db->where('sc.student_qunique_id',$studentId);
		$query = $this->db->get();
		//echo $this->db->last_query(); die;
		if($query->num_rows() > 0):
			return $query->result();
