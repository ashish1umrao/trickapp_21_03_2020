...
class Ciqrcode
{
	...
	public function generate($params = array(),$PNG_TEMP_DIR = '') {
		...
		if (isset($params['savename'])) {
			...
			//$PNG_TEMP_DIR = './assets/student_pic/'.DIRECTORY_SEPARATOR.'temp'.DIRECTORY_SEPARATOR;
                        
			//$filename = $PNG_TEMP_DIR.$params['data'].'.png';
			$filename = $PNG_TEMP_DIR.$params['savename'];
			QRcode::png($params['data'], $filename, $level, $size, 2);
			return $params['savename'];
		}elseif(isset($params['saveTeacherQrcode'])){
			$level = 'L';
			if (isset($params['level']) && in_array($params['level'], array('L','M','Q','H'))) $level = $params['level'];
			...
			$PNG_TEMP_DIR = './assets/teacher_image/'.DIRECTORY_SEPARATOR.'qrcode'.DIRECTORY_SEPARATOR;
			$filename = $PNG_TEMP_DIR.$params['saveTeacherQrcode'];
			QRcode::png($params['data'], $filename, $level, $size, 2);
			return $params['saveTeacherQrcode'];
		}elseif(isset($params['saveStaffQrcode'])){
			$level = 'L';
			if (isset($params['level']) && in_array($params['level'], array('L','M','Q','H'))) $level = $params['level'];
			...
			//$PNG_TEMP_DIR = './assets/staff_image/'.DIRECTORY_SEPARATOR.'qrcode'.DIRECTORY_SEPARATOR;
			$filename = $PNG_TEMP_DIR.$params['saveStaffQrcode'];
			QRcode::png($params['data'], $filename, $level, $size, 2);
			return $params['saveStaffQrcode'];
		} else {
			$level = 'L';
			if (isset($params['level']) && in_array($params['level'], array('L','M','Q','H'))) $level = $params['level'];
