...
$(document).ready(function(){ 
	...
	if(CURRENTMETHOD.indexOf("ddedit")>0){
	   ...
	   var activelink	=	'/index';
	} else if(CURRENTMETHOD.indexOf("ssign")>0){
	   var activelink	=	'/index';
...
if($("#currentPageForm").length) {
	$("#currentPageForm").validate({ 
		rules: {	
			student_f_name: { minlength: 2, maxlength: 50 },
...
//////////////////////////////////   Get Zipcode Data Through Ajax
function get_zipcode_data(curobj,state,city,locality,zip)
{
  curobj.closest('.address-parent').find('.selectzip').val('');
  ...
  if(state !='' && locality !=''){
    $.ajax({
      type: 'post',
       url: BASEURL+'login/get_zipcode_data',
      data: {csrf_api_key:csrf_api_value,state:state,city:city,locality:locality,zip:zip},
     success: function(zresponse){ 
          curobj.closest('.address-parent').find('.selectzip').val(zresponse);
      }
    });
  }
}

//////////////////////////////////   Get Section Data Through Ajax
function get_section_data(curobj,classid,sectionid,particularid)
{
   if(classid !=''){  
    $.ajax({
      type: 'post',
       url: BASEURL+'login/get_section_data',
...
//////////////////////////////////   Get Teacher Data Through Ajax
function get_teacher_data(curobj,classid,sectionid,teacherid)
{
   if((classid !='') &&(sectionid !='')){
    $.ajax({
      type: 'post',
	   url: BASEURL+'login/get_teacher_data',
      data: {csrf_api_key:csrf_api_value,classid:classid,sectionid:sectionid,teacherid:teacherid},
	  success: function(zresponse){ 
          curobj.closest('.class-parent').find('#teacher_id').html(zresponse);
      }
    });
  }
}

///////////////////////////////////////////// Get Franchies school and Branch data ////////////////////////////

//////////////////////////////////   Get School Data Through Ajax
function get_school_data(curobj,franchiesid,schoolid)
{
   if(franchiesid !=''){  
    $.ajax({
      type: 'post',
       url: BASEURL+'login/get_school_data',
      data: {csrf_api_key:csrf_api_value,franchiesid:franchiesid,schoolid:schoolid},
     success: function(zresponse){ 
			curobj.closest('.class-parent').find('#school_id').html(zresponse);
      }
    });
  }
}
 
 //////////////////////////////////   Get Branch Data Through Ajax
 function get_branch_data(curobj,franchiesid,schoolid,branchid)
{ //alert(franchiesid); alert(schoolid); alert(sectionid);
   if((franchiesid !='') &&(schoolid !='')){
    $.ajax({
      type: 'post',
       url: BASEURL+'login/get_branch_data',
      data: {csrf_api_key:csrf_api_value,franchiesid:franchiesid,schoolid:schoolid,branchid:branchid},
     success: function(zresponse){ 
          curobj.closest('.class-parent').find('#branch_id').html(zresponse);
      }
    });
  }
}
