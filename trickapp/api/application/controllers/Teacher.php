				...
				if($branchData <> ""):  
					...
					if($teacherData <> ""):  
						$classQuery				=	"SELECT clas.encrypt_id as classId, clas.class_name as className 
											 		 ...
											 		 INNER JOIN ".getTablePrefix()."classes AS clas ON pst.class_id=clas.encrypt_id
		...
		if(requestAuthenticate(APIKEY)):  
			...
			elseif($this->input->post('subjectId') == ''):
				...
				if($branchData <> ""):  
					...
					$teacherData				=	$this->common_model->getDataByQuery('single',$teacherQuery);
					if($teacherData <> ""):  
						$syllabusQuery			=	"SELECT clsyl.syllabus,clsyl.encrypt_id as syllabusId,clsyl.readed
					...
					if($teacherData <> ""):
						...
						$studentQuery			=	"SELECT stud.student_qunique_id as studentId, CONCAT(stud.student_f_name,' ',stud.student_l_name) as studentName,
													 stud.student_image  as studentImage
											 		 ...
											 		 INNER JOIN ".getTablePrefix()."students AS stud ON stucls.student_qunique_id=stud.student_qunique_id
						...
						if($clstuData <> ""):
							$studentQuery			=	"SELECT stud.student_qunique_id as studentId, CONCAT(stud.student_f_name,' ',stud.student_l_name) as studentName,
														 stud.student_image  as studentImage
												 		 ...
												 		 GROUP By stucls.student_qunique_id";
							...
							if($attendData <> ""):
								...
								foreach($attendanceArray as $attendanceValue):
									//if($attendanceValue['attendance'] == 'Present'):
										...
										$param['attedence_status']   		= 	$attendanceValue['attendance'];
			...
			elseif($this->input->post('attendanceType') == ''):
				...
				if($branchData <> ""):  
					...
					if($teacherData <> ""):
						...
						$studentQuery			=	"SELECT stud.student_qunique_id as studentId, CONCAT(stud.student_f_name,' ',stud.student_l_name) as studentName,
													 stud.student_image  as studentImage,att.attedence_status
											 		 FROM ".getTablePrefix()."student_class AS stucls
											 		 INNER JOIN ".getTablePrefix()."students AS stud ON stucls.student_qunique_id=stud.student_qunique_id
											 		 INNER JOIN sms_student_attendance as att on stud.student_qunique_id=att.student_qunique_id
													 WHERE stucls.board_id = '".$boardId."' AND stucls.class_id = '".$classId."' 
											 		 AND stucls.section_id = '".$sectionId."' AND stucls.session_year = '".CURRENT_SESSION."' AND att.date LIKE '%".$date."%'";
						...
						if($studentData <> ""):  
							...
							foreach($studentData as $studentValue):		
								if($studentValue['attedence_status']==null)
								{
									$studentValue['attedence_status'] = '';
								}
								...
								$stuAttrArray[$i]['status']			=	$studentValue['attedence_status'];
								$i++;
						endforeach;
	...
	{	
		...
		if(requestAuthenticate(APIKEY)):  
			...
			elseif($this->input->post('message') == ''):
				...
				$branchData					=	$this->common_model->getDataByQuery('single',$branchQuery);
				if($branchData <> ""):  
					$teacherQuery			=	"SELECT encrypt_id FROM ".getTablePrefix()."users 
												WHERE encrypt_id = '".$userId."' AND user_type = '".$userType."'";
//////////////////////////////// SEND SMS IN SINGLE TABLE /////////////////////////////////////////////////////////
					$param['franchise_id']   			= 	$branchData['franchiseId'];
					$param['school_id']   				= 	$branchData['schoolId'];
                    $param['branch_id']                 = 	$branchData['branchId'];
                    ...
                    $param['from_id']                  	= 	$userId;
                    $param['login_type']                = 	$userType;
                    $param['creation_date']             = 	$date.' '.$time;
                    $param['created_by']              	= 	$userId;
                   // $param['message_content']         = 	addslashes($message);
                              
					$teacherData				=	$this->common_model->getDataByQuery('single',$teacherQuery);
					$ulastInsertId				=	$this->common_model->addData('sms_templete_sms',$param);
					$Uparam['encrypt_id']		=	manojEncript($ulastInsertId);
					$Uwhere['id']				=	$ulastInsertId;
					$ParentID 					= 	$Uparam['encrypt_id']; 
					$this->common_model->editDataByMultipleCondition('sms_templete_sms',$Uparam,$Uwhere);
					//print "<pre>"; print_r($param); die;  
////////////////////////////// END SEND SMS IN SINGLE TABLE //////////////////////////////////////////////////////
					if($teacherData <> ""):
						$allstudentData		=	json_decode($studentData);
		                if($allstudentData): 
			                foreach($allstudentData as $allstudentValue): 								
                               // $param['message_to_type']           = 	'TeacherParent';
                                $typeparam['parent_id']           		= 	$ParentID;
                                $typeparam['to_id']             		= 	$allstudentValue;
                                $typeparam['message']             		= 	addslashes($message);
                                $typeparam['creation_date']          	= 	$date.' '.$time;
                                $typeparam['created_by']              	= 	$userId;
                                $typeparam['status']            		=   'Y';
					...
					//print $subQuery; die;
					$data['FEDITDATA'] = $this->common_model->getDataByQuery('single', $subQuery);
					$ContactNumber = $data['FEDITDATA']['user_mobile'];	
					$name = $data['FEDITDATA']['student_name'];



                        $data['SMSDATA']	=	$this->sms_model->send_sms($ContactNumber,$message);
//					   	print "<pre>"; print_r($data['SMSDATA']->MessageData[0]->MessageId); die;
                 ...
                 $alastInsertId = $this->common_model->addData('sms_multiple_student_msg',$typeparam);	
				 $Uparam['encrypt_id']		=	manojEncript($alastInsertId);
				 $Uparam['Number']			=	$ContactNumber;
				 $Uparam['MessageId']		=	$MsgID;
				 $Uparam['message']			=	$message;
				 $Uparam['parent_id']		=	$ParentID;
				 $Uwhere['id']				=	$alastInsertId;						 
				 $this->common_model->editDataByMultipleCondition('sms_multiple_student_msg',$Uparam,$Uwhere);	 
							endforeach;
						endif;
			...
			elseif($this->input->post('messageType') == ''):
				...
				if($userData <> ""): 
						...
						if($messageType == 'sent'):
							...
							$toMessQuery = "SELECT multi.encrypt_id as messageId,multi.message as messageContent,
											CONCAT(users.user_f_name,' ',users.user_m_name,' ',users.user_l_name) as messageFrom,
											CONCAT(stu.student_f_name,' ',stu.student_m_name,' ',stu.student_l_name) as messageTo,
											multi.creation_date as messageDate,temp_sms.from_id 
											FROM sms_templete_sms as temp_sms 
											INNER JOIN sms_users as users on temp_sms.from_id=users.encrypt_id 
											INNER JOIN sms_multiple_student_msg as multi on temp_sms.encrypt_id=multi.parent_id
											LEFT JOIN sms_students as stu on multi.to_id=stu.student_qunique_id
											WHERE temp_sms.from_id='".$userId."'";	
			...
			elseif($this->input->post('messageType') == ''):
				...
				if($branchData <> ""):  
					...
					if($userData <> ""): 
						$messageData 		=	array();
						if($messageType == 'sent'):							
							$toMessQuery = "SELECT multi.encrypt_id as messageId,multi.message as messageContent,				CONCAT(users.user_f_name,' ',users.user_m_name,' ',users.user_l_name) as messageFrom,CONCAT(stu.student_f_name,' ',stu.student_m_name,' ',stu.student_l_name) as messageTo,multi.creation_date as messageDate,temp_sms.from_id 
							FROM sms_templete_sms as temp_sms 
							INNER JOIN sms_users as users on temp_sms.from_id=users.encrypt_id 
							INNER JOIN sms_multiple_student_msg as multi on temp_sms.encrypt_id=multi.parent_id
							LEFT JOIN sms_students as stu on multi.to_id=stu.student_qunique_id";
							if($userType=='Teacher'){
							$toMessQuery.=" WHERE temp_sms.from_id='".$userId."'";	
							}else{
							$toMessQuery.=" WHERE multi.to_id='".$userId."'";		
							}
				 	 

						//print $toMessQuery; die; 
							$messageData 	=	$this->common_model->getDataByQuery('multiple',$toMessQuery);
							//echo "<pre>"; print_r($messageData); die;
						elseif($messageType == 'received'):
							$fromMes1Query	=	"SELECT mess.encrypt_id as messageId, mess.message_content as messageContent, 'Admin' as messageFrom, 
						...
						if($messageData):  
							//print "<pre>"; print_r($messageData); die;
							$result['messageList']=	$messageData;						
							echo outPut(lang('SUCESS_STATUS'),lang('SUCESS_MESSAGE'),$result);
						else:
							echo outPut(lang('ERROR_STATUS'),lang('ERROR_MESSAGE'),$result);
				...
				if($branchData <> ""):  
					...
					if($userData <> ""): 
						$homewkQuery	=	"SELECT clss.encrypt_id as classId, clss.class_name as className,
											 ...
											 hwork.home_work_from_date as homeWorkFromDate,
											 hwork.home_work_to_date as homeWorkToDate,hwork.readed
	...
	{	
		...
		if(requestAuthenticate(APIKEY)):  
			...
			elseif($this->input->post('subjectId') == ''):
				...
				$branchData					=	$this->common_model->getDataByQuery('single',$branchQuery);
				if($branchData <> ""):  
					$userQuery				=	"SELECT usr.encrypt_id, CONCAT(usr.user_f_name,' ',usr.user_l_name) as userName,usr.user_type
												 FROM ".getTablePrefix()."users as usr
												 WHERE usr.encrypt_id = '".$userId."' AND usr.user_type = '".$userType."'";
					$userData				=	$this->common_model->getDataByQuery('single',$userQuery);
					if($userData <> ""): 
						$assignQuery	=	"SELECT clss.encrypt_id as classId, clss.class_name as className,
											 ...
											 assign.assignments_from_date as assignmentFromDate,
											 assign.assignments_to_date as assignmentToDate,assign.readed
                        ...
                        if($assignData):  
                            if($userType == Teacher):
                                 $result['assignmentList']	=	$assignData;
                                 	...
                                 	$result['ParentassignmentList']	=	$assignData;
						    endif;
		...
		if(requestAuthenticate(APIKEY)): 
			...
			elseif($this->input->post('teacher_id') == ''):
				...
				$PERIODCOUNT	=	$this->common_model->getDataByQuery('multiple',$subQuery);	
				...
				foreach ($branchData as $timetableData) {
					if(!empty($infoDay['working_day_name'])){
					if($infoDay['working_day_name']=='Monday'){
					 $Monday   =   
					 	array('class_name'=>$timetableData['class_name'],
                              'class_section_name'=>$timetableData['class_section_name'],
                               'subject_name'=>$timetableData['subject_name'],
                               'class_period_name'=>$timetableData['class_period_name']);
				...
				$response['TeacherTimeTableList'] = $result['TeacherTimeTableList'];
	...
	/* * *********************************************************************
	 * * Function name : getstudentattendancehistorylist
	 * * Developed By : Ashish UMrao
	 * * Purpose  : This function used for get student attendance history list
	 * * Date : 28 JUNE 2019
	 * * **********************************************************************/
	public function getstudentattendancehistorylist()
	{	
		$this->generatelogs->putLog('APP',logOutPut($_POST));
		$result 							= 	array();
		$returnData 						= 	array();
		if(requestAuthenticate(APIKEY)):  //print_r($_POST); die;
			if($this->input->post('studentquniqueID') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('STUDENT_UNIQUE_ID'),$result);
			else:	
				$studentquniqueID					=	($this->input->post('studentquniqueID'));	
					$attedencehistory				=	"SELECT date,time,attedence_status
													 FROM ".getTablePrefix()."student_attendance 
													 WHERE student_qunique_id = '".$studentquniqueID."'";
					
						$attedencehistorylist				=	$this->common_model->getDataByQuery('multiple',$attedencehistory);
					//echo "<pre>"; print_r($attedencehistorylist); die;
					if($attedencehistorylist<>""):  
						$result['studentattendancehistorylist']	=	$attedencehistorylist;						
						echo outPut(lang('SUCESS_STATUS'),lang('SUCESS_MESSAGE'),$result);
					else:
						echo outPut(lang('ERROR_STATUS'),lang('ERROR_MESSAGE'),$result);
					endif;
	...
	{	
		...
		if(requestAuthenticate(APIKEY)):  
			...
			elseif($this->input->post('subjectId') == ''):
				...
				$sectionId					=	trim($this->input->post('sectionId'));
				$subjectId					=	trim($this->input->post('subjectId'));	
				$syllabusId					=	trim($this->input->post('syllabusId'));
				...
				if($branchData <> ""):  
					...
					$teacherData				=	$this->common_model->getDataByQuery('single',$teacherQuery);
					if($teacherData <> ""):  
						$syllabusQuery			=	"SELECT clsyl.syllabus,clsyl.readed
											 		 FROM ".getTablePrefix()."class_syllabus AS clsyl
											 		 WHERE clsyl.board_id = '".$boardId."' AND clsyl.class_id = '".$classId."' 
											 		 AND clsyl.subject_id = '".$subjectId."' AND clsyl.session_year = '".CURRENT_SESSION."'";
						//print $syllabusQuery; die; 
						...
						if($syllabusData <> ""):  
							$uParam['readed']	=	'Y';
							$this->common_model->editData('sms_class_syllabus',$uParam,'encrypt_id',$syllabusId);
	 ...
	 * * Date : 14 JANUARY 2019
	 * * **********************************************************************/
	public function updateHomeWorkList()
	{	
		$this->generatelogs->putLog('APP',logOutPut($_POST));
		$result 							= 	array();
		$returnData 						= 	array();
		if(requestAuthenticate(APIKEY)):  
			...
			elseif($this->input->post('subjectId') == ''):
				...
				$fromDate					=	trim($this->input->post('fromDate'));	
				$toDate						=	trim($this->input->post('toDate'));
				$homeWorkId					=	trim($this->input->post('homeWorkId'));	
				...
				if($branchData <> ""):  
					...
					if($userData <> ""): 
                        ...
                        if($homewkData):  
							$uParam['readed']	=	'Y';
							$this->common_model->editData('sms_home_work',$uParam,'encrypt_id',$homeWorkId);
	...
	{	
		...
		if(requestAuthenticate(APIKEY)):  
			...
			elseif($this->input->post('subjectId') == ''):
				...
				$fromDate					=	trim($this->input->post('fromDate'));	
				$toDate						=	trim($this->input->post('toDate'));
				$assignmentId				=	trim($this->input->post('assignmentId'));	
					...
					if($userData <> ""): 
                        ...
                        if($assignData):  
                            if($userType == Teacher):
								...
								$uParam['readed']	=	'Y';
								$this->common_model->editData('sms_assignments',$uParam,'encrypt_id',$assignmentId);
								...
								echo json_encode($response);
                                 	...
                                 	//$result['ParentassignmentList']	=	$assignData;
								$uParam['readed']	=	'Y';
							$this->common_model->editData('sms_home_work',$uParam,'encrypt_id',$assignmentId);
