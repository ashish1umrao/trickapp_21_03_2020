...
class User extends CI_Controller {
	...
	{	
		...
		$result 							= 	array();
		$returnData 						= 	array();
		if(requestAuthenticate(APIKEY)):  //print_r($_POST); die;
			if($this->input->post('branchCode') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('BRANCH_CODE_EMPTY'),$result);
			...
			elseif($this->input->post('token_no') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('TOKEN_EMPTY'),$result);
				...
				$token_no					=	trim($this->input->post('token_no'));
					
				$branchQuery				=	"SELECT brd.admin_franchise_id as franchise_id, 
												 schd.encrypt_id as school_id, schd.admin_name as school_name,clas.encrypt_id as class_id,clas.class_name as class_name,
												 brd.encrypt_id as branch_id, brd.branch_code as branch_code, brd.admin_name as branch_name,csection.encrypt_id as sectionid,brd.*
												 FROM ".getTablePrefix()."admin as brd 
												 LEFT JOIN ".getTablePrefix()."admin as schd ON  brd.admin_school_id=schd.encrypt_id
												 LEFT JOIN ".getTablePrefix()."classes as clas ON  brd.admin_school_id=clas.school_id
												 LEFT JOIN ".getTablePrefix()."users as u ON  brd.admin_school_id=u.school_id
												 LEFT JOIN ".getTablePrefix()."class_section as csection ON  u.encrypt_id=csection.class_teacher_id
												 ...
												 GROUP BY u.encrypt_id";
					 //echo "<pre>"; print_r($branchQuery); die;
				 $branchData					=	$this->common_model->getDataByQuery('single',$branchQuery);
				 if($branchData <> ""):  
					$userQuery				=	"SELECT encrypt_id, employee_id, user_f_name, user_m_name, user_l_name, user_email,user_mobile,token_no,
					...
					if($userData <> ""):  
						...
						if($decryptUserPassword == $userPassword):
							if($userData['status'] == 'A'):								
								...
								$uParam['token_no']			=	$token_no;
								$uParam['user_last_login']	=	currentDateTime();
								$this->common_model->editData('users',$uParam,'encrypt_id',$userData['encrypt_id']);

								// Image Get Here School

			$imageQuery =	"SELECT admin_id,encrypt_id,admin_image,admin_image_app  
			FROM `sms_admin` WHERE `encrypt_id` LIKE '".$branchData['school_id']."'";
					 $imageData	=	$this->common_model->getDataByQuery('single',$imageQuery);
								// End Here 
								if($userData['status']=='A'){
								...
								$returnData['ClassID']		=	$branchData['class_id']?stripslashes($branchData['class_id']):'';
								$returnData['ClassName']	=	$branchData['class_name']?stripslashes($branchData['class_name']):'';
                                $returnData['SectionID']	=	$branchData['sectionid']?stripslashes($branchData['sectionid']):'';
                                $returnData['schoolEmail']	=	$branchData['admin_email_id']?stripslashes($branchData['admin_email_id']):'';
                                $returnData['schoolContactNo']	=	$branchData['admin_mobile_number']?stripslashes($branchData['admin_mobile_number']):'';
                                $returnData['schoolAddress']	=	$branchData['admin_address']?stripslashes($branchData['admin_address']):'';
                                $returnData['schoolLocality']	=	$branchData['admin_locality']?stripslashes($branchData['admin_locality']):'';
                                $returnData['schoolCity']	=	$branchData['admin_city']?stripslashes($branchData['admin_city']):'';
                                $returnData['schoolState']	=	$branchData['admin_state']?stripslashes($branchData['admin_state']):'';
                                $returnData['schoolZipCode']	=	$branchData['admin_zipcode']?stripslashes($branchData['admin_zipcode']):'';
                                $returnData['schoolImage']	=	$imageData['admin_image']?stripslashes($imageData['admin_image']):'';
                                $returnData['schoolImageApp']	=	$imageData['admin_image_app']?stripslashes($imageData['admin_image_app']):'';
    
								$returnData['userId']		=	$userData['encrypt_id']?$userData['encrypt_id']:'';
								$returnData['EmployeeId']	=	$userData['employee_id']?$userData['employee_id']:'';
								$returnData['token_no']		=	$userData['token_no']?$userData['token_no']:'';
								...
								$returnData['userMobileNo']	=	$userData['user_mobile']?stripslashes($userData['user_mobile']):'';
								$returnData['userLastLogin']=	$userData['user_last_login']?date('M, d Y',strtotime($userData['user_last_login'])):'';
								$returnData['status']=			$userData['status']?stripslashes($userData['status']):'';
								...
								elseif($returnData['userType'] == 'Parent'):
									...
									$childQuery				=	"SELECT stu.student_qunique_id, stu.student_f_name, stu.student_m_name, stu.student_l_name,stu.student_image, 
																 clss.encrypt_id as class_id, clss.class_name,
																 sec.encrypt_id as section_id, sec.class_section_name as section_name,branch.board_id
																 ...
																 LEFT JOIN ".getTablePrefix()."student_branch as branch on stu.student_qunique_id=branch.student_qunique_id
																 WHERE spar.parent_id = '".$returnData['userId']."' AND spar.session_year = '".CURRENT_SESSION."'
																 GROUP BY stu.student_qunique_id";
									//print $childQuery; die;
									$childData				=	$this->common_model->getDataByQuery('multiple',$childQuery);
									//print "<pre>"; print_r($childData); die;
									$returnData['boardId']	=	$childData[0]['board_id'];
									 $returnData['SectionID'] = $childData[0]['section_id'];
									...
									//	$returnData['childImage']	=	$childData;
...
if($userType=='Parent'){
	$parentQuery =	"SELECT users.encrypt_id,stu_add.* FROM sms_users users LEFT JOIN sms_student_parent parent on users.encrypt_id=parent.parent_id LEFT JOIN sms_student_address stu_add on parent.student_qunique_id=stu_add.student_qunique_id WHERE users.encrypt_id='".$userId."'";
	$parentData	=	$this->common_model->getDataByQuery('single',$parentQuery);
//print "<pre>"; print_r($parentData); die;
	$returnData['userAddress']	=	$parentData['student_c_address']?stripslashes($parentData['student_c_address']):'';
						$returnData['userLocality']	=	$parentData['student_c_locality']?stripslashes($parentData['student_c_locality']):'';
						$returnData['userCity']		=	$parentData['student_c_city']?stripslashes($parentData['student_c_city']):'';
						$returnData['userState']	=	$parentData['student_c_state']?stripslashes($parentData['student_c_state']):'';
						$returnData['userZipcode']	=	$parentData['student_c_zipcode']?stripslashes($parentData['student_c_zipcode']):'';
	...
	$childQuery	= "SELECT stu.student_qunique_id, stu.student_f_name, 
					stu.student_m_name, stu.student_l_name,stu.student_image, 
					 clss.encrypt_id as class_id, clss.class_name,
					 ...
					 LEFT JOIN ".getTablePrefix()."classes as clss ON scls.class_id=clss.encrypt_id
					 LEFT JOIN ".getTablePrefix()."class_section as sec ON scls.section_id=sec.encrypt_id
					 WHERE spar.parent_id = '".$userId."' AND spar.session_year = '".CURRENT_SESSION."'
					 GROUP BY stu.student_qunique_id";
									//print $childQuery; die;
									$childData				=	$this->common_model->getDataByQuery('multiple',$childQuery);
									if($childData <> ""):
						...
						$result['userProfileData']			=	$returnData;						
				...
				if($branchData <> ""):  
					...
					if($userData <> ""): 
						...
						$holidayQuery				=	"SELECT holiday_id as eventId, purpose as eventTitle,
														 venue as eventVenue, 'Holidays' as eventType,
					...
					//	print $holidayQuery; die;
						...
						if($userType=='Teacher'):						
							$eventsQuery			=	"SELECT event_id as eventId, purpose as eventTitle,
														 ...
														 CONCAT(DATE_FORMAT(from_date, '%d-%m-%Y'),'T',sms_event.time) as eventStart, 
														 CONCAT(DATE_FORMAT(to_date, '%d-%m-%Y'),'T',sms_event.time) as eventEnd 
						...
						if($eventData):  
							//print "<pre>"; print_r($eventData); die;	
					...
					if($userData <> ""): 
						if($userType == 'Teacher'):
							...
							elseif($feedbackType == 'received'):
								$feedbackQuery	=	"SELECT fback.encrypt_id as feedbackId, CONCAT(stud.student_f_name,' ',stud.student_l_name) as feedbackFrom,stud.student_image as studentpicture,udetails.user_pic as teacherpicture,
													 ...
													 LEFT JOIN ".getTablePrefix()."students as stud ON fback.sender_id=stud.student_qunique_id
													  LEFT JOIN ".getTablePrefix()."user_detils as udetails ON udetails.encrypt_id=sect.class_teacher_id
													 LEFT JOIN ".getTablePrefix()."subject as subj ON fback.to_subject_id=subj.encrypt_id
							...
							elseif($this->input->post('childId') == ''):
								...
								elseif($feedbackType == 'received'):
									$feedbackQuery	=	"SELECT fback.encrypt_id as feedbackId, CONCAT(usr.user_f_name,' ',usr.user_l_name) as feedbackFrom, 
														 CONCAT(stud.student_f_name,' ',stud.student_l_name) as feedbackTO, clss.class_name as feedbackClass,
														 sect.class_section_name as feedbackSection, '' as feedbackSubject,stud.student_image as studentpicture,udetails.user_pic as teacherpicture, 
														 fback.message as feedbackMessage, fback.creation_date as messageDate
														 FROM ".getTablePrefix()."feedback as fback
														 LEFT JOIN ".getTablePrefix()."classes as clss ON fback.to_class_id=clss.encrypt_id
														 LEFT JOIN ".getTablePrefix()."class_section as sect ON fback.to_section_id=sect.encrypt_id
														 LEFT JOIN ".getTablePrefix()."students as stud ON fback.to_student_id=stud.student_qunique_id
	...
	{	
		...
		$result 							= 	array();
		$returnData 						= 	array();
		if(requestAuthenticate(APIKEY)):  //echo "<pre>"; print_r($_POST); die;
			if($this->input->post('branchCode') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('BRANCH_CODE_EMPTY'),$result);
			elseif($this->input->post('userType') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('USER_TYPE_EMPTY'),$result);
			...
			elseif($this->input->post('boardId') == ''):
				...
				if($branchData <> ""):  
					...
					if($userData <> ""): 
						if($userType == 'Teacher'):
							$notBordQuery	=	"SELECT nboard.encrypt_id as noticeBoardId,nboard.notice_title, nboard.message as noticeBoardMessage,
												  nboard.image as noticeBoardImage, nboard.creation_date as noticeBoardDate,nboard.readed
	...
	{			
				...
				if($branchData <> ""):  
					...
					if($userData <> ""): 
						if($userType == 'Teacher'):
							...
							$notisQuery	=	"SELECT noti.encrypt_id as notificationId,noti.title, noti.message as notificationMessage,
												 noti.creation_date as notificationDate,noti.readed
		...
		if(requestAuthenticate(APIKEY)): 
			...
			if($this->input->post('user_id') == ''):
				...
				$subQuery   =	"SELECT stu.student_id,stu.encrypt_id,stu.student_f_name,stu_class.class_id,stu_class.section_id,cl.class_name,
					sc.class_section_name,sub.subject_name,days.working_day_name,
					period.class_period_name,sub_teach.period_id,sub_teach.working_day_id,
					COALESCE(users.user_f_name,' ',users.user_m_name,' ',users.user_l_name) as teacher_name 
FROM sms_students stu LEFT JOIN sms_student_class stu_class on stu.student_qunique_id=stu_class.student_qunique_id 
LEFT JOIN sms_classes as cl on stu_class.class_id=cl.encrypt_id
LEFT JOIN sms_class_section as sc on stu_class.section_id=sc.encrypt_id
LEFT JOIN sms_period_subject_teacher sub_teach on stu_class.section_id=sub_teach.section_id
LEFT JOIN sms_subject sub on sub_teach.subject_id=sub.encrypt_id
LEFT JOIN sms_working_days days on sub_teach.working_day_id=days.encrypt_id
LEFT JOIN sms_class_period period on sub_teach.period_id=period.encrypt_id
LEFT JOIN sms_users users on sub_teach.teacher_id=users.encrypt_id
WHERE stu.student_qunique_id='".$user_id."'";
				//print $subQuery; die;		

				$subQueryCountPeriod = "SELECT stu.student_id,stu.encrypt_id,stu.student_f_name,stu_class.class_id,stu_class.section_id,cl.class_name,
					sc.class_section_name,sub.subject_name,days.working_day_name,
					period.class_period_name,sub_teach.period_id,sub_teach.working_day_id,
					COALESCE(users.user_f_name,' ',users.user_m_name,' ',users.user_l_name) as teacher_name 
FROM sms_students stu LEFT JOIN sms_student_class stu_class on stu.student_qunique_id=stu_class.student_qunique_id 
...
LEFT JOIN sms_class_period period on sub_teach.period_id=period.encrypt_id
LEFT JOIN sms_users users on sub_teach.teacher_id=users.encrypt_id
WHERE stu.student_qunique_id='".$user_id."' GROUP BY period.class_period_name";
		
				$PERIODCOUNT	=	$this->common_model->getDataByQuery('multiple',$subQueryCountPeriod);	
				//print_r(sizeof($PERIODCOUNT)); die;		
				$TotalPeriod = sizeof($PERIODCOUNT);


				$TIMETABLE	=	$this->common_model->getDataByQuery('multiple',$subQuery);	
				//print "<pre>"; print_r($TIMETABLE); die;
				...
				$SaturdayArr = array();
				$response = array();
				foreach ($TIMETABLE as $TIMETABLEINFO) {							
					if(!empty($TIMETABLEINFO['working_day_name'])){
					if($TIMETABLEINFO['working_day_name']=='Monday'){
					 $Monday   =   
					 	array('subject_name'=>$TIMETABLEINFO['subject_name'],
                              'class_period_name'=>$TIMETABLEINFO['class_period_name'],
                          	  'period_teacher_name'=>$TIMETABLEINFO['teacher_name']);
					array_push($MondayArr, $Monday); 	
					}
					if($TIMETABLEINFO['working_day_name']=='Tuesday'){
					 $Tuesday   =   
					 	array('subject_name'=>$TIMETABLEINFO['subject_name'],
                              'class_period_name'=>$TIMETABLEINFO['class_period_name'],
                           	  'period_teacher_name'=>$TIMETABLEINFO['teacher_name']);
					...
					if($TIMETABLEINFO['working_day_name']=='Wednesday'){
					 $Wednesday   =   
					 	array('subject_name'=>$TIMETABLEINFO['subject_name'],
                               'class_period_name'=>$TIMETABLEINFO['class_period_name'],
                          	   'period_teacher_name'=>$TIMETABLEINFO['teacher_name']);
					...
					if($TIMETABLEINFO['working_day_name']=='Thursday'){
					 $Thursday   =   
					 	array('subject_name'=>$TIMETABLEINFO['subject_name'],
                               'class_period_name'=>$TIMETABLEINFO['class_period_name'],
                               'period_teacher_name'=>$TIMETABLEINFO['teacher_name']);
					...
					if($TIMETABLEINFO['working_day_name']=='Friday'){
					 $Friday   =   
					 	array('subject_name'=>$TIMETABLEINFO['subject_name'],
                               'class_period_name'=>$TIMETABLEINFO['class_period_name'],
                               'period_teacher_name'=>$TIMETABLEINFO['teacher_name']);
					...
					if($TIMETABLEINFO['working_day_name']=='Saturday'){
					 $Saturday   =   
					 	array('subject_name'=>$TIMETABLEINFO['subject_name'],
                               'class_period_name'=>$TIMETABLEINFO['class_period_name'],
                               'period_teacher_name'=>$TIMETABLEINFO['teacher_name']);
				...
				if($TIMETABLE <> ""): 
					$result['StudentTimeTableList']=	array('Monday'=> $MondayArr,
															'Tuesday'=> $TuesdayArr,
					...
					//echo outPut(lang('SUCESS_STATUS'),lang('SUCESS_MESSAGE'),$result);	
					$response['success'] = 1;
					$response['message'] = lang('SUCESS_MESSAGE');
					$response['TotalPeriod'] = $TotalPeriod;
					$response['StudentTimeTableList'] = $result['StudentTimeTableList'];
		...
		if(requestAuthenticate(APIKEY)): 
			...
			if($this->input->post('user_id') == ''):
				...
				$subQuery   =	"SELECT stu.student_id,stu.encrypt_id,stu.student_f_name,stu_class.class_id,stu_class.section_id,cl.class_name,
					sc.class_section_name,sub.subject_name,days.working_day_name,
					period.class_period_name,sub_teach.period_id,sub_teach.working_day_id,
					COALESCE(users.user_f_name,' ',users.user_m_name,' ',users.user_l_name) as teacher_name,sub_teach.subject_id 
FROM sms_students stu LEFT JOIN sms_student_class stu_class on stu.student_qunique_id=stu_class.student_qunique_id 
LEFT JOIN sms_classes as cl on stu_class.class_id=cl.encrypt_id
LEFT JOIN sms_class_section as sc on stu_class.section_id=sc.encrypt_id
LEFT JOIN sms_period_subject_teacher sub_teach on stu_class.section_id=sub_teach.section_id
...
WHERE stu.student_qunique_id='".$user_id."'"; 
if(!empty($teacher_id)){
$subQuery.=" AND users.encrypt_id='".$teacher_id."'";	
}
$subQuery.=" GROUP BY sub_teach.subject_id";
				//print $subQuery; die;										
				$TIMETABLE	=	$this->common_model->getDataByQuery('multiple',$subQuery);	
				//print "<pre>"; print_r($TIMETABLE); die;
				$response = array();
				...
				foreach ($TIMETABLE as $TIMETABLEINFO) {							
					if(!empty($TIMETABLEINFO)){					
					 $SubjectInfo   =  						
					 	array('subject_id'=>$TIMETABLEINFO['subject_id'],
                              'subject_name'=>$TIMETABLEINFO['subject_name']);						
						array_push($response, $SubjectInfo); 		
					
				}
						
				
			}
					//print "<pre>"; print_r($response[0]['subject_id']); die;
				if(!empty($response[0]['subject_id'])){
					if($TIMETABLE <> ""): 
					$result['StudentSubjectList']=	$response;
						echo outPut(lang('SUCESS_STATUS'),lang('SUCESS_MESSAGE'),$result);					
					else:
						echo outPut(lang('ERROR_STATUS'),lang('INVALID_BRANCH_CODE'),$result);
					endif;
...
public function getStudentTeacherList(){
		...
		if(requestAuthenticate(APIKEY)): 
			...
			if($this->input->post('user_id') == ''):
				...
				$subQuery   =	"SELECT stu.student_id,stu.encrypt_id,stu.student_f_name,stu_class.class_id,stu_class.section_id,cl.class_name,
                                    sc.class_section_name,sub.subject_name,days.working_day_name,period.class_period_name,sub_teach.period_id,sub_teach.working_day_id,COALESCE(users.user_f_name,' ',users.user_m_name,' ',users.user_l_name) as teacher_name,sub_teach.subject_id,sub_teach.teacher_id,user_det.user_pic 
                                    FROM sms_students stu LEFT JOIN sms_student_class stu_class on stu.student_qunique_id=stu_class.student_qunique_id 
                                    LEFT JOIN sms_classes as cl on stu_class.class_id=cl.encrypt_id
                                    LEFT JOIN sms_class_section as sc on stu_class.section_id=sc.encrypt_id
                                    LEFT JOIN sms_period_subject_teacher sub_teach on stu_class.section_id=sub_teach.section_id
                                    ...
                                    LEFT JOIN sms_class_period period on sub_teach.period_id=period.encrypt_id
                                    LEFT JOIN sms_users users on sub_teach.teacher_id=users.encrypt_id
                                    LEFT JOIN sms_user_detils as user_det on users.encrypt_id=user_det.user_id
                                    LEFT JOIN sms_teacher_subject as tsubjectlist on sub_teach.teacher_id=tsubjectlist.teacher_id
                                    WHERE stu.student_qunique_id='".$user_id."' GROUP BY sub_teach.teacher_id";
				//print $subQuery; die;										
				$TIMETABLE	=	$this->common_model->getDataByQuery('multiple',$subQuery);	
				//print "<pre>"; print_r($TIMETABLE); die;
				$response = array();				
				foreach ($TIMETABLE as $TIMETABLEINFO) {							
					if(!empty($TIMETABLEINFO)){					
					 $SubjectInfo   =   
					 	array('teacher_id'=>$TIMETABLEINFO['teacher_id'],
                              'teacher_name'=>$TIMETABLEINFO['teacher_name'],
                          	  'teacher_image'=>$TIMETABLEINFO['user_pic'],
                          	  'subject_name'=>$TIMETABLEINFO['subject_name']);
					array_push($response, $SubjectInfo); 											
				}
				...
				if(!empty($response[0]['teacher_id'])){
					if($TIMETABLE <> ""): 
					$result['StudentTeacherList']=	$response;
						echo outPut(lang('SUCESS_STATUS'),lang('SUCESS_MESSAGE'),$result);					
					else:
						echo outPut(lang('ERROR_STATUS'),lang('INVALID_BRANCH_CODE'),$result);
					endif;
...
/* * *********************************************************************
	 * * Function name : getbusroute
	 * * Developed By : Ashish UMrao
	 * * Purpose  : This function used for get busroute
	 * * Date : 22 JUNE 2019
	 * * **********************************************************************/
	...
	{	
		...
		if(requestAuthenticate(APIKEY)):  //echo "<pre>"; print_r($_POST); die;
			...
			elseif($this->input->post('boardId') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('BOARD_ID_EMPTY'),$result);
			elseif($this->input->post('schoolid') == ''):
					echo outPut(lang('ERROR_STATUS'),lang('SCHOOL_EMPTY'),$result);
			else:	
				$branchCode					=	trim($this->input->post('branchCode'));	
				$userType					=	trim($this->input->post('userType'));	
				$userId						=	trim($this->input->post('userId'));
				...
				if($branchData <> ""):  
					...
					if($teacherData <> ""):  
						$getbusroute			=	"SELECT r.route_name,r.route_short_name,v.vehicle_no,v.no_of_seat,v.max_seat_extend,vtype.vehicle_type_name,rdetails.stop_name,rdetails.stop_latitude,rdetails.stop_longitude,rdetails.pickup_time,rdetails.drop_time,rdetails.fee,
						                             rassign.driver_id,CONCAT(u.user_f_name,' ',u.user_m_name,' ',u.user_l_name) as driver_name,u.user_email as driver_email,u.user_mobile as driverMobileNo,
						                            rassign.conductor_id,CONCAT(cuser.user_f_name,' ',cuser.user_m_name,' ',cuser.user_l_name) as conductor_name,cuser.user_mobile as conductorMobileNo,
						                            rassign.attendant_id,CONCAT(auser.user_f_name,' ',auser.user_m_name,' ',auser.user_l_name) as attendant_name,auser.user_email as attendant_email,auser.user_mobile as attendentMobileNo
														 FROM ".getTablePrefix()."route AS r
														 LEFT JOIN ".getTablePrefix()."route_assign as rassign ON r.encrypt_id=rassign.route_id
														 LEFT JOIN ".getTablePrefix()."route_detail as rdetails ON r.encrypt_id=rdetails.route_id
														 LEFT JOIN ".getTablePrefix()."branch_boards as bboard ON r.school_id=bboard.school_id
														 LEFT JOIN ".getTablePrefix()."users as u ON bboard.school_id=u.school_id
														 LEFT JOIN ".getTablePrefix()."users as us ON rassign.driver_id=us.encrypt_id
														 LEFT JOIN ".getTablePrefix()."users as cuser  ON rassign.conductor_id=cuser.encrypt_id
														 LEFT JOIN ".getTablePrefix()."users as auser  ON rassign.attendant_id=auser.encrypt_id
														 LEFT JOIN ".getTablePrefix()."vehicle as v ON r.encrypt_id=rdetails.route_id
														 LEFT JOIN ".getTablePrefix()."vehicle_types as vtype ON v.vehicle_type=vtype.encrypt_id
											 		 	WHERE bboard.board_id = '".$boardId."' AND u.user_type like '%Driver%' ";
						//print $getbusroute; die; 
						$BusrouteDetails			=	$this->common_model->getDataByQuery('single',$getbusroute);
						if($BusrouteDetails <> ""):  
							$result['busrouteData']=	$BusrouteDetails;						
							echo outPut(lang('SUCESS_STATUS'),lang('SUCESS_MESSAGE'),$result);
						else:
							echo outPut(lang('ERROR_STATUS'),lang('ERROR_MESSAGE'),$result);
						endif;
...
/* * *********************************************************************
	 * * Function name : getReportCardSession
	 * * Developed By :  Jitendra Singh
	 * * Purpose  : This function used for get report card session
	 * * Date : 28 JUNE 2019
	 * * **********************************************************************/
	function getReportCardSession(){
		...
		if(requestAuthenticate(APIKEY)): 
			...
			if($this->input->post('schoolId') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('SCHOOL_EMPTY'),$result);
			elseif($this->input->post('branchId') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('BRANCH_EMPTY'),$result);			
			elseif($this->input->post('boardId') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('BOARD_ID_EMPTY'),$result);
			else:  	
				$subQuery   =	"SELECT ass.*,class.class_name,sec.class_section_name 
						  ...
						  WHERE ass.school_id = '".$this->input->post('schoolId')."' 
						  AND ass.branch_id = '".$this->input->post('branchId')."' 
						  AND ass.board_id = '".$this->input->post('boardId')."' AND ass.status='Y'";
				//print $subQuery; die;										
				$TIMETABLE	=	$this->common_model->getDataByQuery('multiple',$subQuery);	
				//print "<pre>"; print_r($TIMETABLE); die;
				$response = array();				
				foreach ($TIMETABLE as $TIMETABLEINFO) {							
					if(!empty($TIMETABLEINFO)){					
					 $SubjectInfo   =   
					 	array('session_id'=>$TIMETABLEINFO['encrypt_id'],
                              'session_name'=>$TIMETABLEINFO['name']);
					array_push($response, $SubjectInfo); 											
				}
				...
				if($TIMETABLE <> ""): 
					$result['StudentSessionList']=	$response;
					echo outPut(lang('SUCESS_STATUS'),lang('SUCESS_MESSAGE'),$result);					
				else:
					echo outPut(lang('ERROR_STATUS'),lang('INVALID_BRANCH_CODE'),$result);
				endif;
    ...
    /* * *********************************************************************
	 * * Function name : getStudentReportCard
	 * * Developed By :  Jitendra Singh
	 * * Purpose  : This function used for get report card
	 * * Date : 28 JUNE 2019
	 * * **********************************************************************/
		...
		elseif($this->input->post('sectionId') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('SECTION_ID_EMPTY'),$result);
		elseif($this->input->post('sessionId') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('SESSION_ID_EMPTY'),$result);	
		elseif($this->input->post('studentId') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('STUDENT_ID_EMPTY'),$result);						
			else:  
				...
				$subQuery   =	"SELECT stud.student_id,stud.encrypt_id as stu_encrypt_id,stu_cl.student_roll_no, 
								CONCAT(stud.student_f_name,' ',stud.student_l_name) as student_name,ass.max_mark,subj.subject_name,
								stu_mark.mark FROM sms_student_class stu_cl 
								LEFT JOIN sms_students as  stud on stu_cl.student_qunique_id=stud.student_qunique_id 
								...
								WHERE stu_cl.school_id='".$schoolId."' AND stu_cl.branch_id='".$branchId."' 
								AND stu_cl.board_id='".$boardId."' AND stu_cl.class_id='".$classId."' 
								AND stu_cl.section_id='".$sectionId."' AND ass.encrypt_id='".$sessionId."' 
								AND stud.student_qunique_id='".$studentId."' GROUP BY subj.encrypt_id";
				//print $subQuery; die;										
				$TIMETABLE	=	$this->common_model->getDataByQuery('multiple',$subQuery);	
				//print "<pre>"; print_r($TIMETABLE); die;
				$response = array();
				...
				foreach ($TIMETABLE as $TIMETABLEINFO) {							
					if(!empty($TIMETABLEINFO)){	
					 ...
					 $SubjectInfo   =   
					 	array('student_id'=>$TIMETABLEINFO['stu_encrypt_id'],
                              'student_name'=>$TIMETABLEINFO['student_name'],
                          	  'student_roll_no'=>$TIMETABLEINFO['student_roll_no'],
                          	  'max_mark'=>$TIMETABLEINFO['max_mark'],
                          	  'mark'=>$TIMETABLEINFO['mark'],
                          	  'subject_name'=>$TIMETABLEINFO['subject_name']);
						$totalmarks = $totalmarks+$TIMETABLEINFO['max_mark'];
						$totalobtainmarks = $totalobtainmarks+$TIMETABLEINFO['mark'];
					array_push($response, $SubjectInfo); 											
				}
				
			}
				$percentage = $totalobtainmarks;
				
				$totalpercentage = $percentage*100/$totalmarks;
				//$totalpercentage = 33;
				
				//$shortfoll = 100;
				if($totalpercentage < 100 && $totalpercentage >= 60){
				   $Division = 'First'; 
				}elseif($totalpercentage <= 59 && $totalpercentage >= 50){
				   $Division = 'Second'; 
				}elseif($totalpercentage <= 49 && $totalpercentage >= 33){
				   $Division = 'Third'; 
				}elseif($totalpercentage <= 32 && $totalpercentage >= 0){
				   $Division = 'Fail'; 
				}
				...
				if($TIMETABLE <> ""): 
					$result['StudentReportCardList']=	$response;
					$result['TotalMarks'] = $totalmarks;
					$result['ObtainMarks'] = $totalobtainmarks;
	...
	/* * *********************************************************************
	 * * Function name : loginwithqrcode
	 * * Developed By : Manoj Kumar
	 * * Purpose  : This function used for user Login with QR Code
	 * * Date : 07 DECEMBER 2018
	 * * **********************************************************************/
public function loginwithqrcode()
	{	
		...
		$returnData 						= 	array();
		if(requestAuthenticate(APIKEY)):  
			if($this->input->post('studentregistrationNo') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('STUDENT_REGIS_NO'),$result);
			else:
				$StudentregistrationNo		=	trim($this->input->post('studentregistrationNo'));
				
				$branchQuery				=	"SELECT brd.admin_franchise_id as franchise_id, 
												 schd.encrypt_id as school_id, schd.admin_name as school_name,clas.encrypt_id as class_id,clas.class_name as class_name,
												 brd.encrypt_id as branch_id, brd.branch_code as branch_code, brd.admin_name as branch_name,csection.encrypt_id as sectionid
												 FROM ".getTablePrefix()."admin as brd 
												 LEFT JOIN ".getTablePrefix()."admin as schd ON  brd.admin_school_id=schd.encrypt_id
												 LEFT JOIN ".getTablePrefix()."classes as clas ON  brd.admin_school_id=clas.school_id
												 LEFT JOIN ".getTablePrefix()."users as u ON  brd.admin_school_id=u.school_id
												 LEFT JOIN ".getTablePrefix()."class_section as csection ON  u.encrypt_id=csection.class_teacher_id
												 LEFT JOIN ".getTablePrefix()."student_branch as sbranch ON  brd.admin_school_id=sbranch.school_id
												 WHERE sbranch.student_registration_no = '".$StudentregistrationNo."' AND brd.admin_type = 'Branch'
												 AND brd.admin_level = '3' AND brd.status ='A'";
					 
					$branchData					=	$this->common_model->getDataByQuery('single',$branchQuery);
					if($branchData <> ""):  
						$userQuery				=	"SELECT u.encrypt_id, u.employee_id, u.user_f_name, u.user_m_name, u.user_l_name, u.user_email,u.user_mobile,u.token_no,
					                            u.user_phone, u.user_type, u.user_last_login, u.status
												 FROM ".getTablePrefix()."users as u
												  LEFT JOIN ".getTablePrefix()."student_branch as sbranch ON  u.school_id=sbranch.school_id 
												  WHERE sbranch.student_registration_no = '".$StudentregistrationNo."' AND user_type='Parent'";
					...
					if($userData <> ""):  
							if($userData['status'] == 'A'):
								...
								if($returnData['userType'] == 'Parent'):
									$returnData['boardId']	=	'';
									$returnData['boardName']=	'';
									$returnData['boardList']=	array();
											$childQuery				=	"SELECT sbranch.student_registration_no,stu.student_qunique_id, stu.student_f_name, stu.student_m_name, stu.student_l_name,stu.student_image
																		,clss.encrypt_id as class_id, clss.class_name,sec.encrypt_id as section_id, sec.class_section_name as section_name
																 FROM ".getTablePrefix()."student_branch as sbranch
																LEFT JOIN sms_students as stu ON  sbranch.student_qunique_id=stu.student_qunique_id
																 LEFT JOIN ".getTablePrefix()."classes as clss ON sbranch.school_id=clss.school_id
																 LEFT JOIN ".getTablePrefix()."class_section as sec ON clss.encrypt_id=sec.class_id  
																WHERE sbranch.student_registration_no = '".$StudentregistrationNo."'
																GROUP BY sbranch.student_registration_no";
									//print $childQuery; die;
									$childData				=	$this->common_model->getDataByQuery('multiple',$childQuery);
									if($childData <> ""):
    ...
    /* * *********************************************************************
	 * * Function name : getStudentfeereportcard
	 * * Developed By : Ashish Umrao
	 * * Purpose  : This function used for get Student fee report card
	 * * Date : 10 JULY 2019
	 * * **********************************************************************/
	public function getStudentfeesreportcard(){
		...
		if(requestAuthenticate(APIKEY)): 
			//print_r($_POST); die;
			if($this->input->post('studentuniqueID') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('STUDENT_UNIQUEID'),$result);
			else:  
							
				$studentuniqueID					=	trim($this->input->post('studentuniqueID'));	
				
				$subQuery   =	"SELECT fhead.*, fheading.heading_type,sclasssection.class_section_name as section_name,c.class_name,s.student_f_name
								FROM ".getTablePrefix()."student_final_paybal_amount as fhead 
								LEFT JOIN ".getTablePrefix()."student_class stu_class ON fhead.student_id=stu_class.student_qunique_id 
								LEFT JOIN ".getTablePrefix()."fee_heading as fheading ON fhead.fee_head_name=fheading.encrypt_id
								LEFT JOIN ".getTablePrefix()."students as s ON fhead.student_id=s.student_qunique_id
								LEFT JOIN ".getTablePrefix()."student_class as sclass ON fhead.student_id=sclass.student_qunique_id
								LEFT JOIN ".getTablePrefix()."class_section as sclasssection ON sclass.section_id=sclasssection.encrypt_id
								LEFT JOIN ".getTablePrefix()."classes as c ON sclass.class_id=c.encrypt_id
								WHERE fhead.student_id = '".$studentuniqueID."' GROUP BY fhead.student_id";
				//print $subQuery; die;
				$TIMETABLE	=	$this->common_model->getDataByQuery('multiple',$subQuery);	
				//print "<pre>"; print_r($TIMETABLE); die;
				$response = array();
				foreach ($TIMETABLE as $TIMETABLEINFO) {
					$SubjectInfo   =   
					 	array(
							 'Per_month_fee'=>$TIMETABLEINFO['per_month_fee'],
                              'Fine'=>$TIMETABLEINFO['due_fine'],
                          	  'total_paybal_amount'=>$TIMETABLEINFO['total_paybal_amount']);
					array_push($response, $SubjectInfo); 										
				}
				$subQuery1   =	"SELECT paymonth.month_name
				FROM ".getTablePrefix()."student_final_paybal_amount as paymonthname
				LEFT JOIN ".getTablePrefix()."fee_paybal_month as paymonth ON paymonthname.id=paymonth.student_pay_fee_id  
				WHERE paymonthname.student_id = '".$studentuniqueID."'";
				$monthname	=	$this->common_model->getDataByQuery('multiple',$subQuery1);	
				//print "<pre>"; print_r($TIMETABLE1); die;

				
				$response1 = array();
				foreach ($monthname as $NameMonth) {
					
					$MonthInfo   =   
					 	array(
							 'Pay_month_name'=>$NameMonth['month_name'],
                             );
					array_push($response1, $MonthInfo); 										
				}

				if($TIMETABLE <> ""): 
					$result['StudentfeepayDetails']=	$response;
					$result['feespaymonthlist']=	$response1;
					echo outPut(lang('SUCESS_STATUS'),lang('SUCESS_MESSAGE'),$result);					
				else:
					echo outPut(lang('ERROR_STATUS'),lang('INVALID_BRANCH_CODE'),$result);
				endif;
	...
	{	
		...
		if(requestAuthenticate(APIKEY)):  
			...
			elseif($this->input->post('boardId') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('BOARD_ID_EMPTY'),$result);
			elseif($this->input->post('notificationId') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('NOTIFICATION ID IS EMPTY'),$result);	 
			else:  
				...
				$userId						=	trim($this->input->post('userId'));	
				$boardId					=	trim($this->input->post('boardId'));
				$notificationId				=	trim($this->input->post('notificationId'));
					...
					if($userData <> ""): 
						if($userType == 'Teacher'):
							...
							$notisQuery	=	"SELECT noti.encrypt_id as notificationId,noti.title, noti.message as notificationMessage,
												 ...
												 FROM ".getTablePrefix()."notification as noti
												 WHERE noti.franchise_id = '".$branchData['franchiseId']."' AND noti.school_id = '".$branchData['schoolId']."' 
											 	 AND noti.branch_id = '".$branchData['branchId']."' AND noti.board_id = '".$boardId."' AND noti.encrypt_id='".$notificationId."'
											 	 ORDER BY UNIX_TIMESTAMP(noti.creation_date) DESC";	
	                        ...
	                        if($notisData):  
								// UPDATE HERE								
								$uParam['readed']	=	'Y';
								$this->common_model->editData('sms_notification',$uParam,'encrypt_id',$notificationId);
	 ...
	 * * Date : 09 JANUARY 2019
	 * * **********************************************************************/
	public function updateNoticeboardStatus()
	{	
		...
		if(requestAuthenticate(APIKEY)):  //echo "<pre>"; print_r($_POST); die;
			...
			elseif($this->input->post('boardId') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('BOARD_ID_EMPTY'),$result);
			elseif($this->input->post('noticeboardId') == ''):
				echo outPut(lang('ERROR_STATUS'),'NOTICEBOARD ID IS EMPTY',$result);	
			else:  
				$branchCode					=	trim($this->input->post('branchCode'));	
				$userType					=	trim($this->input->post('userType'));	
				$userId						=	trim($this->input->post('userId'));	
				$boardId					=	trim($this->input->post('boardId'));
				$noticeboardId				=	trim($this->input->post('noticeboardId'));
					...
					if($userData <> ""): 
						if($userType == 'Teacher'):
							$notBordQuery	=	"SELECT nboard.encrypt_id as noticeBoardId,nboard.notice_title, nboard.message as noticeBoardMessage,
												 ...
												 WHERE nboard.franchise_id = '".$branchData['franchiseId']."' AND nboard.school_id = '".$branchData['schoolId']."' 
											 	 AND nboard.branch_id = '".$branchData['branchId']."' AND nboard.board_id = '".$boardId."' 
												 AND nboard.encrypt_id='".$noticeboardId."'
											 	 AND (nboard.visibility = 'All' OR nboard.visibility = 'Teacher')
											 	 ORDER BY UNIX_TIMESTAMP(nboard.creation_date) DESC";
						    //print $notBordQuery; die;
							$notBordData 	=	$this->common_model->getDataByQuery('multiple',$notBordQuery);
	                        if($notBordData):  
								// UPDATE HERE								
								$uParam['readed']	=	'Y';
								$this->common_model->editData('sms_notice_board',$uParam,'encrypt_id',$noticeboardId);									
								$response['success'] = 1;
								$response['message'] = "Record is updated!";								
								echo json_encode($response);
							else:
						...
						elseif($userType == 'Parent'):
							...
							elseif($this->input->post('sectionId') == ''):
								...
								$notBordQuery	=	"SELECT nboard.encrypt_id as noticeBoardId,nboard.notice_title, nboard.message as noticeBoardMessage,
													 ...
													 FROM ".getTablePrefix()."notice_board as nboard
													 WHERE nboard.franchise_id = '".$branchData['franchiseId']."' AND nboard.school_id = '".$branchData['schoolId']."' 
												 	 AND nboard.branch_id = '".$branchData['branchId']."' AND nboard.board_id = '".$boardId."' AND nboard.encrypt_id='".$noticeboardId."'
												 	 AND (nboard.class_id = 'All' OR nboard.class_id = '".$classId."')
												 	 AND (nboard.section_id = 'All' OR nboard.section_id = '".$sectionId."')
												 	 AND (nboard.visibility = 'All' OR nboard.visibility = 'Parent')
												 	 ORDER BY UNIX_TIMESTAMP(nboard.creation_date) DESC";
	...
	{	
		...
		if(requestAuthenticate(APIKEY)):  
			if($this->input->post('branchCode') == ''):
				echo outPut(lang('ERROR_STATUS'),lang('BRANCH_CODE_EMPTY'),$result);
			elseif($this->input->post('userContact') == ''):
				echo outPut(lang('ERROR_STATUS'),'USER CONTACT IS EMPTY',$result);
			elseif($this->input->post('userType') == ''):
				echo outPut(lang('ERROR_STATUS'),'USER TYPE IS EMPTY',$result);	
			else:
				$branchCode					=	trim($this->input->post('branchCode'));	
				$userContact				=	trim($this->input->post('userContact'));
				$userType				=	trim($this->input->post('userType'));	
				$branchQuery				=	"SELECT su.* FROM sms_admin as sa LEFT JOIN sms_users as su 
												on sa.admin_school_id=su.school_id WHERE sa.branch_code='".$branchCode."' 
												AND su.user_mobile='".$userContact."' AND su.user_type='".$userType."' 
												AND su.status='A'";
				//print $branchQuery; die;
				$branchData					=	$this->common_model->getDataByQuery('single',$branchQuery);
				
				if($branchData <> ""):  
					$encrypt_id	= $branchData['encrypt_id']; 	
					$OTP	= getRandomOTP();
					$message = "Your OTP is ".$OTP."";
					$data['SMSDATA']	 = $this->sms_model->send_sms($userContact,$message);
					
					// UPDATE HERE								
					$uParam['otp']			=	$OTP;
					$uParam['verifyotp']	=	0;
					$this->common_model->editData('sms_users',$uParam,'encrypt_id',$encrypt_id);				
									
					$response['success'] = 1;
					$response['message'] = "Your otp is send in your registered mobile number!";
					echo json_encode($response);	
				else:
					//echo outPut(lang('ERROR_STATUS'),lang('INVALID_BRANCH_CODE'),$result);
					echo outPut(lang('ERROR_STATUS'),'INVALID DETAILS',$result);
				endif;
			endif;
		else:
			echo outPut(lang('ERROR_STATUS'),lang('INVALID_APIKEY'),$result);
	...
	{	
		...
		if(requestAuthenticate(APIKEY)):  
			...
			/* elseif($this->input->post('userType') == ''):
				...
				$branchQuery				=	"SELECT su.* FROM sms_admin as sa LEFT JOIN sms_users as su 
												on sa.admin_school_id=su.school_id WHERE sa.branch_code='".$branchCode."' 
												AND su.user_mobile='".$userContact."' AND su.status='A'";
				//print $branchQuery; die;
				$branchData					=	$this->common_model->getDataByQuery('single',$branchQuery);
				
				if($branchData <> ""):  
					$encrypt_id	= $branchData['encrypt_id']; 	
					$DBOTP = $branchData['otp'];
					if($userOTP!=$DBOTP){
						$response['success'] = 0;
					...
					$response['success'] = 1;
					$response['message'] = "Your otp is successfully verified!";
					echo json_encode($response);	
					}
				else:
					//echo outPut(lang('ERROR_STATUS'),lang('INVALID_BRANCH_CODE'),$result);
	...
	{	
		...
		if(requestAuthenticate(APIKEY)):  
			...
			elseif($this->input->post('userType') == ''):
				...
				if($branchData <> ""):  
					$user_password	= 	$this->common_model->encript_password($userPassword);
					$encrypt_id	= $branchData['encrypt_id']; 	
										
					// UPDATE HERE													
					$uParam['user_password']	=	$user_password;
					$uParam['first_login']		=	1;
					$this->common_model->editData('sms_users',$uParam,'encrypt_id',$encrypt_id);				
									
					$response['success'] = 1;
					$response['message'] = "Your password is changed!";
					echo json_encode($response);					
				else:
					//echo outPut(lang('ERROR_STATUS'),lang('INVALID_BRANCH_CODE'),$result);
					echo outPut(lang('ERROR_STATUS'),'INVALID DETAILS',$result);
	 ...
	 * * Date : 07 DECEMBER 2018
	 * * **********************************************************************/
	public function userauthenticateOnline()
	{	
		$this->generatelogs->putLog('APP',logOutPut($_POST));
		$result 							= 	array();
		$returnData 						= 	array();
		if(requestAuthenticate(APIKEY)):  //print_r($_POST); die;
			...
			elseif($this->input->post('token_no') == ''):
				...
				$branchQuery				=	"SELECT brd.admin_franchise_id as franchise_id, 
				 ...
				 if($branchData <> ""):  
					$userQuery				=	"SELECT encrypt_id, employee_id, user_f_name, user_m_name, user_l_name, user_email,user_mobile,token_no,
					                             user_password, user_phone, user_type, user_last_login,status,first_login
												 FROM ".getTablePrefix()."users 
												 WHERE user_mobile = '".$userContact."' AND user_type = '".$userType."'";
					//echo "<pre>"; print_r($userQuery); die;
					$userData				=	$this->common_model->getDataByQuery('single',$userQuery);
					...
					if($userData['first_login']==0){
						$decryptUserPassword	=	$userContact;
					}else{
						$decryptUserPassword	=	$this->user_model->decryptsPassword($userData['user_password']);
					}						
